...
void AlarmParaInit(void*pManage)
{
	...
	printf("$$$$$$$$$$$$$$$$$$AlarmParaInit id:%d, nModMask: 0x%x\n", getpid(), psBizManager->sBizPara.nModMask);//psBizManager->nNVROrDecoder
...
static int reloadHdd(disk_manager *hddHdr, SBizDevInfo *pNewDisk)
{
	...
	for(i = 0; i < MAX_PARTITION_NUM; i++)
	{
		...
		{
			//psHddInfo->is_partition_exist[i] = 1;
			psHddInfo->total += (get_partition_total_space(&psHddInfo->ptn_index[i])/1000);
			psHddInfo->free += (get_partition_free_space(&psHddInfo->ptn_index[i])/1000);
		}
		else// if(ret == 0)
		{
			//psHddInfo->is_partition_exist[i] = 0;
...
s32 DoBizInit(SBizManager* psBizManager, SBizInitPara* psBizPara)
{
	...
	if(flag_First)
	{
		...
		else if((0 == strcmp("NR3564",sDvrInfo.sproductnumber)))
		{
			//
		}
		...
		
		{
			...
			//nvr.cpp 中远程NVR 报警
			tl_set_AlarmCB(AlarmDeal);
			
			// debug by lanston
			if(0 == strcasecmp("R9624T",sDvrInfo.sproductnumber))
			{
				tl_hslib_init_c(TL_BOARD_TYPE_R9624T, init_param, sDvrInfo.nVidMainNum);
			}
			else if(0 == strcasecmp("R9632S",sDvrInfo.sproductnumber))
			{
				tl_hslib_init_c(TL_BOARD_TYPE_R9632SL, init_param, sDvrInfo.nVidMainNum);
			}
			else if(0 == strcasecmp("R9624SL",sDvrInfo.sproductnumber))
			{
				tl_hslib_init_c(TL_BOARD_TYPE_R9624SL, init_param, sDvrInfo.nVidMainNum);
			}
			else if(0 == strcasecmp("R9616S",sDvrInfo.sproductnumber))
			{
				tl_hslib_init_c(TL_BOARD_TYPE_R9616S, init_param, sDvrInfo.nVidMainNum);
			}
			else if(0 == strcasecmp("R9608S",sDvrInfo.sproductnumber))
			{
				tl_hslib_init_c(TL_BOARD_TYPE_R9608S, init_param, sDvrInfo.nVidMainNum);
			}
			else if(0 == strcasecmp("R9504S",sDvrInfo.sproductnumber))
			{
				#if defined(_JMV_)//csp modify
				tl_hslib_init_c(TL_BOARD_TYPE_R9504S_V22_JMS, init_param, sDvrInfo.nVidMainNum);
				#elif defined(_JUAN_)//csp modify
				tl_hslib_init_c(TL_BOARD_TYPE_R2004MX, init_param, sDvrInfo.nVidMainNum);
				#else
				//csp modify
				tl_hslib_init_c(TL_BOARD_TYPE_R9504S, init_param, sDvrInfo.nVidMainNum);
			...
			{
				#if defined(_JMV_)//csp modify
				tl_hslib_init_c(TL_BOARD_TYPE_R9508S_V22_JMS, init_param, sDvrInfo.nVidMainNum);
				#elif defined(_JUAN_)//csp modify
				tl_hslib_init_c(TL_BOARD_TYPE_R2008MX, init_param, sDvrInfo.nVidMainNum);
				#else
				tl_hslib_init_c(TL_BOARD_TYPE_R9508S, init_param, sDvrInfo.nVidMainNum);
				#endif
			}
			else if(0 == strcmp("R9516S",sDvrInfo.sproductnumber))
			{
				#if defined(_R9516S_CX25838_CX25828_)//csp modify
				tl_hslib_init_c(TL_BOARD_TYPE_R9516_CX25838_CX25828, init_param, sDvrInfo.nVidMainNum);
				#elif defined(longse)//csp modify
				tl_hslib_init_c(TL_BOARD_TYPE_R9516, init_param, sDvrInfo.nVidMainNum);
				#else
				tl_hslib_init_c(TL_BOARD_TYPE_R9516_TW2826, init_param, sDvrInfo.nVidMainNum);
				#endif
			}
			#ifdef CHIP_HISI3531
			else if(0 == strcmp("R3104HD",sDvrInfo.sproductnumber))
			{
				tl_hslib_init_c(TL_BOARD_TYPE_R9800_MA, init_param, sDvrInfo.nVidMainNum);
			}
			else if(0 == strcmp("R3110HDW",sDvrInfo.sproductnumber))
			{
				tl_hslib_init_c(TL_BOARD_TYPE_R9800_DA, init_param, sDvrInfo.nVidMainNum);
			}
			...
			{
				tl_hslib_init_c(TL_BOARD_TYPE_R9816_SA, init_param, sDvrInfo.nVidMainNum);
			}
			...
			{
				tl_hslib_init_c(TL_BOARD_TYPE_NR3116, init_param, sDvrInfo.nVidMainNum);
			}
			else if(0 == strcmp("NR3132",sDvrInfo.sproductnumber))
			{
				tl_hslib_init_c(TL_BOARD_TYPE_NR3132, init_param, sDvrInfo.nVidMainNum);
			}
			else if(0 == strcmp("NR3124",sDvrInfo.sproductnumber))
			{
				tl_hslib_init_c(TL_BOARD_TYPE_NR3124, init_param, sDvrInfo.nVidMainNum);
			}
			#endif
			#ifdef CHIP_HISI3521
			else if(0 == strcmp("NR3564",sDvrInfo.sproductnumber))
			{
				//printf("%s CHIP_HISI3521\n", __func__);
				tl_hslib_init_c(TL_BOARD_TYPE_NR3564, init_param, sDvrInfo.nVidMainNum);
			}
			else if(0 == strcmp("R2104",sDvrInfo.sproductnumber))
			{
				tl_hslib_init_c(TL_BOARD_TYPE_R9704S, init_param, sDvrInfo.nVidMainNum);
			}
			...
			{
				tl_hslib_init_c(TL_BOARD_TYPE_R9708S, init_param, sDvrInfo.nVidMainNum);
			}
			...
			else if(0 == strcmp("R2116",sDvrInfo.sproductnumber))
			{
				...
				tl_hslib_init_c(TL_BOARD_TYPE_R9716D, init_param, sDvrInfo.nVidMainNum);
			}
			else if(0 == strcmp("R2016",sDvrInfo.sproductnumber))
			{
				//csp modify 20130506
				//init_param |= (0x01<<17);
				tl_hslib_init_c(TL_BOARD_TYPE_R9716S, init_param, sDvrInfo.nVidMainNum);
			}
			else if(0 == strcmp("NR2116",sDvrInfo.sproductnumber))
			{
				//printf("%s 3 init_param = 0x%08x\n",__func__, init_param);
				tl_hslib_init_c(TL_BOARD_TYPE_NR2116, init_param, sDvrInfo.nVidMainNum);
			}
			else if(0 == strcmp("NR1004",sDvrInfo.sproductnumber))
			{
				tl_hslib_init_c(TL_BOARD_TYPE_NR1004, init_param, sDvrInfo.nVidMainNum);
			}
			else if(0 == strcmp("NR1008",sDvrInfo.sproductnumber))
			{
				tl_hslib_init_c(TL_BOARD_TYPE_NR1008, init_param, sDvrInfo.nVidMainNum);
			}
			#endif
			else
			{
				switch(sDvrInfo.nVidMainNum)
				{
					case 4:
					{
			    		tl_hslib_init_c(TL_BOARD_TYPE_R9504_4D1, init_param, sDvrInfo.nVidMainNum);
						//printf("here,init TL_BOARD_TYPE_R9504_4D1......\n");
					} break;
					case 8:
					{
			    		tl_hslib_init_c(TL_BOARD_TYPE_R9508, init_param, sDvrInfo.nVidMainNum);
	...
	if(1)// (!(0 == strcasecmp("R9632S",sDvrInfo.sproductnumber)))
	{
		...
		if(0 == strcasecmp("NR2116",sDvrInfo.sproductnumber) || 
			...
			0 == strcasecmp("NR3564",sDvrInfo.sproductnumber) ||
...
//#endif
	...
	else if (0 == strcmp("NR3564",sDvrInfo.sproductnumber))//csp modify
	{
		sBizPlayback.nChnMaskOfD1 = 0xffffffff;//???
	}
	...
	sPara.nNVROrDecoder	= sDvrInfo.nNVROrDecoder;	//跃天: 1 nvr，2 轮巡解码器，3 切换解码器
			...
			case EM_BIZMSG_RESUME_CONFIG:
	         ...
	         case EM_BIZMSG_PLAYBACK_CTRL:
                ...
                case EM_BIZMSG_SYSCOMPLEX_BREAK_BACKUP:
                    SysComplexBreakBackup((SBizBackup *)sBizMsgHead.pData);
                    break;
...
s32 DoBizGetPara(u32 bDefault,SBizPara* psBizPara)
{
	...
	switch(psBizPara->sBizParaTarget.emBizParaType)
	{
		...
		case EM_BIZ_SNAP_PARA:
			return ConfigGetSnapChnPara(bDefault, (SBizIPCameraPara *)psBizPara->pData, psBizPara->sBizParaTarget.nChn);
		//录像布防
		case EM_BIZ_SCHEDULE_RECTIMING:
