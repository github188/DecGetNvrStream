...
)
{
	...
	if(0 == ConfigGetDvrInfo(0, &sDvrInfo))
	{
		...
		pDevInfo->nNVROrDecoder 	= sDvrInfo.nNVROrDecoder;
...
)
{
	...
	ifly_search_ipc_t *ipcSeach = &pCmd->sReq.search_ipc;
	SCPIpcInfo *pSearchIpc = &pRslt->sBasicInfo.sIpcInfo;	
...
)
{
	...
	for(i=0;i<GetVideoMainNum();i++)
	{
		...
		{	
			...
			//modify by yaogang for get nvr stream
			if (ipcam.protocol_type == 0x100)//nvr
			{
				pSearchIpc->para_log[count].max_nvr_chn = ipcam.main_audio_sw;//NVR最大通道数
				pSearchIpc->para_log[count].req_nvr_chn = ipcam.sub_audio_sw;//NVR自己的通道号，指示要申请NVR 的哪一路码流
				//printf("%s req_nvr_chn: %d, max_nvr_chn: %d\n", 
				//	__func__, pSearchIpc->para_log[count].req_nvr_chn, pSearchIpc->para_log[count].max_nvr_chn);
			}
...
)
{
	...
	ret = BizGetPara(&bizTar, (void *)&ipcam);
	if (ret)
	{
		printf("BizGetPara failed, ret: %d\n", ret);
		return ;
	}
	...
	if (ipc_info->protocol_type == 0x100)//nvr
	{
		ipcam.main_audio_sw = ipc_info->max_nvr_chn;//NVR最大通道数
		ipcam.sub_audio_sw = ipc_info->req_nvr_chn;//NVR自己的通道号，指示要申请NVR 的哪一路码流
		//printf("%s 1 req_nvr_chn: %d, max_nvr_chn: %d\n", 
		//	__func__, camera->req_nvr_chn, camera->max_nvr_chn);
	}

#if 0
	struct in_addr in;
	in.s_addr = ipcam.dwIp;
	printf("SetIpc dec_chn: %d, NVR IP= %s, enable: %d, protocol: 0x%x, nvr max chn: %d, chn: %d\n", 
				ipcam.channel_no, inet_ntoa(in), ipcam.enable, ipcam.protocol_type, ipcam.main_audio_sw, ipcam.sub_audio_sw);
#endif

	ret = BizSetPara(&bizTar, (void *)&ipcam);
	if (ret)
	{
		printf("BizGetPara failed, ret: %d\n", ret);
		return ;
...
)
{
	...
	if (camera->channel_no > MaxChn)
	{
		printf("%s ***Error*** channelno%d > MaxChn%d\n", __func__, camera->channel_no, MaxChn);
		return 0;
	}
	printf("%s recv camera info: \n", __func__);
	printf("\t channelno: %d\n", camera->channel_no);
	printf("\t protocol_type: 0x%x\n", camera->protocol_type);
	printf("\t req_nvr_chn: %d\n", camera->req_nvr_chn);
	printf("\t max_nvr_chn: %d\n", camera->max_nvr_chn);
			...
			if(ipcam.enable)
			{
				if((ipcam.protocol_type == camera->protocol_type) && (ipcam.dwIp == camera->dwIp))
				{
					if(ipcam.wPort != camera->wPort)
					{
						...
						if (camera->protocol_type == 0x100)//nvr
						{
							ipcam.main_audio_sw = camera->max_nvr_chn;//NVR最大通道数
							ipcam.sub_audio_sw = camera->req_nvr_chn;//NVR自己的通道号，指示要申请NVR 的哪一路码流
							//printf("%s 1 req_nvr_chn: %d, max_nvr_chn: %d\n", 
							//	__func__, camera->req_nvr_chn, camera->max_nvr_chn);
						}
		...
		for(i=0;i<MaxChn;i++)
		{
			...
			if(!ipcam.enable)
			{
				...
				if (camera->protocol_type == 0x100)//nvr
				{
					ipcam.main_audio_sw = camera->max_nvr_chn;//NVR最大通道数
					ipcam.sub_audio_sw = camera->req_nvr_chn;//NVR自己的通道号，指示要申请NVR 的哪一路码流
					//printf("%s 2 req_nvr_chn: %d, max_nvr_chn: %d\n", 
					//			__func__, camera->req_nvr_chn, camera->max_nvr_chn);
				}
				BizSetPara(&bizTar, (void *)&ipcam);
	...
	else
	{
		...
		if (camera->protocol_type == 0x100)//nvr
		{
			//printf("%s 3 req_nvr_chn: %d, max_nvr_chn: %d\n", 
			//					__func__, camera->req_nvr_chn, camera->max_nvr_chn);

			//printf("%s pre sub_audio_sw: %d, cur sub_audio_sw: %d\n", 
			//					__func__, ipcam.sub_audio_sw, camera->req_nvr_chn);
			ipcam.main_audio_sw = camera->max_nvr_chn;//NVR最大通道数
			ipcam.sub_audio_sw = camera->req_nvr_chn;//NVR自己的通道号，指示要申请NVR 的哪一路码流
...
)
{
	...
	//printf("yg DealRemoteCmd_DelIpc ipc addr : %s, chn: %d, enable: %d\n", 
	//	inet_ntoa(host), ipc_info->channel_no, ipc_info->enable);
	...
	//printf("yg BizGetPara ipc enable: %d\n", ipcam.enable);
...
)
{
	...
	if(ret ==0)
	{
		...
		pRslt->sBasicInfo.patrol_para.nPatrolChnNum = bizPara.nPatrolChnNum;//轮巡通道数16 32 48 64

		printf("%s bizPara.nIsPatrol: %d\n", __func__, bizPara.nIsPatrol);
		printf("%s bizPara.nInterval: %d\n", __func__, bizPara.nInterval);
		printf("%s bizPara.nPatrolMode: %d\n", __func__, bizPara.nPatrolMode);
		printf("%s bizPara.nPatrolChnNum: %d\n", __func__, bizPara.nPatrolChnNum);
...
)
{
	...
	if(ret ==0)
	{
		...
		bizPara.nPatrolChnNum = pCmd->sReq.patrol_para.nPatrolChnNum; //轮巡通道数16 32 48 64
...
)
{
		...
		|| (ModePara != preview_para->ModePara) )
	{
		...
		if((ModePara + PreviewMode) > MaxChn)
		{
			if(MaxChn != ModePara)
			{
				if(MaxChn > PreviewMode)//cw_9508S
					ModePara = MaxChn- PreviewMode;
				else
					ModePara = 0;
			}
		}
