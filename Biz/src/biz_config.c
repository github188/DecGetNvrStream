...
s32 ConfigGetCfgPatrol(u32 bDefault,SBizCfgPatrol *psPara)
{
	...
	if (0 == ret)
	{
		...
		psPara->nPatrolChnNum = sConfig.nPatrolChnNum;
...
s32 ConfigSetCfgPreview(SBizCfgPreview *psPara)
{
	...
	//sPreview.nModeLowChn = 0;//底层使用的通道号(VDEC & VO, 64路解码板使用)
...
s32 ConfigSetCfgPatrol(SBizCfgPatrol *psPara)
{
	...
	{
		sConfig.nIsPatrol = psPara->nIsPatrol;
		sConfig.nPatrolChnNum = psPara->nPatrolChnNum;
		...
		printf("%s nIsPatrol: %d\n", __func__, sConfig.nIsPatrol);
		printf("%s nInterval: %d\n", __func__, sConfig.nInterval);
		printf("%s nPatrolMode: %d\n", __func__, sConfig.nPatrolMode);
		printf("%s nPatrolChnNum: %d\n", __func__, sConfig.nPatrolChnNum);
		...
		if (ret == 0)
		{
			...
			sPatrolPara.nPatrolChnNum = psPara->nPatrolChnNum;
...
//yaogang modify for yuetian private
	//printf("%s cur sub_audio_sw: %d\n", __func__, psPara->sub_audio_sw);
	ipcam.main_audio_sw = psPara->main_audio_sw;//主码流音频开关//NVR最大通道数
	ipcam.sub_audio_sw   = psPara->sub_audio_sw;//次码流音频开关//NVR自己的通道号，指示要申请NVR 的哪一路码流
...
//yaogang modify for yuetian private
	...
	printf("ConfigSetIPCameraPara dec_chn: %d, NVR IP= %s, enable: %d, protocol: 0x%x, nvr max chn: %d, chn: %d\n", 
