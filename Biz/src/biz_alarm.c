...
s32 AlarmInit(u8 nEnable, SBizAlarm* psInitPara)
{
	if(nEnable && psInitPara)
	{
		...
		sAlarmInitPara.nNVROrDecoder = psInitPara->nNVROrDecoder;//跃天: 1 nvr，2 轮巡解码器，3 切换解码器
...
void AlarmDeal(SAlarmCbData* psAlarmCbData)
{
	...
	if(psAlarmCbData)
	{
		...
		switch(psAlarmCbData->emAlarmEvent)
		{
			...
			//nvr.cpp 中远程NVR 报警
		case EM_ALARM_EVENT_RemoteNVR_DISK_LOST:  //硬盘丢失错误
			{
		...
		case EM_ALARM_EVENT_RemoteNVR_DISK_ERR:  //硬盘读写错误
			{
				if(EM_BIZ_EVENT_UNKNOW == sBizEventPara.emType)
				{
					//printf("yg EM_ALARM_EVENT_RemoteNVR_DISK_ERR\n");
		...
		case EM_ALARM_EVENT_RemoteNVR_VLOST:  //视频丢失报警
			{
				if(EM_BIZ_EVENT_UNKNOW == sBizEventPara.emType)
				{
