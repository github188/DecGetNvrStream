...
//yaogang modify 20141202
	...
	//yaogang modify 20150324 //Ô¾Ìì: 1 nvr£¬2 ÂÖÑ²½âÂëÆ÷£¬3 ÇÐ»»½âÂëÆ÷
	u8 nNVROrDecoder;
} SAlarmManager;
...
void CheckAlarmEvent(SAlarmManager *psAlarmManager)
{
	...
	if(count == 6)
	{
		...
		if (psAlarmManager->nNVROrDecoder == 1) //Ô¾Ìì: 1 nvr£¬2 ÂÖÑ²½âÂëÆ÷£¬3 ÇÐ»»½âÂëÆ÷
		{
...
extern int get_disk_err_record();
void CheckAlarmDiskErr(SAlarmEvent *psAlarmEvent, FNALARMCB pfnAlarmCb, int fd485)
{
	int i;
	u32 nEventSatus = 0;
	...
	nEventSatus = get_disk_err_record();
	if (nEventSatus)
...
void CheckAlarmVlosts(SAlarmEvent *psAlarmEvent, FNALARMCB pfnAlarmCb, int fd485)
{
	...
	//printf("videolost = 0x%08x, nChannels = %d################################\n", nEventSatus, psAlarmEvent->nChannels);
...
void AlarmDispatch(SAlarmManager *psAlarmManager)
{
	...
	//´¥·¢Â¼Ïñ
	//yaogang modify 20150324 //Ô¾Ìì: 1 nvr, 2 ½âÂëÆ÷
