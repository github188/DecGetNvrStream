...
)
{
	...
	//xdc
		...
		{
			printf("CTRL_CMD_GETSEACHIPCLIST is start\n");
			ifly_search_ipc_t* ipcSeach = &sReqIns.sReq.search_ipc;;
			...
			printf("CTRL_CMD_GETSEACHIPCLIST is end pAckLen = %d\n",*pAckLen);
		}
		break;
	...
	case CTRL_CMD_GETADDIPCLIST:
		{printf("CTRL_CMD_GETADDIPCLIST is start\n");
			...
			printf("CTRL_CMD_GETADDIPCLIST is end pAckLen = %d\n",*pAckLen);
			
		#endif	
		}
		break;
	case CTRL_CMD_SETIPC:
		{printf("CTRL_CMD_SETIPC is start \n");
			...
			printf("CTRL_CMD_SETIPC dec_chn: %d, NVR IP= %s, chn: %d\n", 
				ipc_info->channel_no, inet_ntoa(in), ipc_info->req_nvr_chn);
			
			(*pCB)(&sReqIns, &sRslt);
			
			if(pAckLen) *pAckLen = 0;
			printf(" CTRL_CMD_SETIPC is end \n");
		}
		break;
	case CTRL_CMD_ADDIPC:
		{printf("CTRL_CMD_ADDIPC is start \n");
			ifly_ipc_info_t*  ipc_info;
			ipc_info = &sReqIns.sReq.ipc_info;
	...
	case CTRL_CMD_DELETEIPC:
		{printf(" CTRL_CMD_DELETEIPC CTRL_CMD_DELETEIPC is start \n");
			...
			//struct in_addr in;
			//in.s_addr = ipc_info->dwIp;
			//printf("yg NVR del dwIP= %s, chn: %d, enable: %d\n", 
			//	inet_ntoa(in), ipc_info->channel_no, ipc_info->enable);
	...
	case CTRL_CMD_GET_PATROL_PARA:
		{
			...
			printf("patrol_para->nPatrolChnNum: %d\n", patrol_para->nPatrolChnNum);
