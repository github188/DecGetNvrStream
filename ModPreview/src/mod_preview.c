...
s32 ModPreviewStart(SPreviewPara* psPreviewPara)
{
	...
	//调整传入参数psPreviewPara->nModePara
	int preMaxChn = 16;//默认预览16画面
...
s32 ModPreviewNext(void)
{
    ...
    u8 nSplitNum = 1;
	int maxChn = g_sPreviewManager.sPatrolPara.nPatrolChnNum;//轮巡通道数
...
//yaogang modify 20141025
	...
	g_sPreviewManager.sPatrolPara.nPatrolChnNum = psPatrolPara->nPatrolChnNum;//轮巡通道数16 32 48 64
			...
			switch (state)
			{
				...
				case 1://模式改变后，等待相应IPC通道I帧到来
				{
					...
					else
					{
						...
						//正常情况下
						if (nModeChangeStamp+1200 < nCurTimeStamp)//1.2秒，I帧间隔1秒
						{
							memset(&sPreviewCB, 0, sizeof(sPreviewCB));	
		...
		*/
				...
				&& ((nCurTimeStamp-nLastTimeStamp >= sPreviewPatrol.nInterval*1000) || flag))
			{
				...
				else
				{
					...
					if(0 != ModPatrolNext())//间接调用ModPreviewStart
					{
