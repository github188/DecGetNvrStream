...
//pw 2010/9/10
//ns : 超时时间单位:秒，必须大于0
//时间控制上不精确，因为select 成功返回没有计算时间
int loopRcvTimeoutNS(SOCKET s, char * buf, unsigned int rcvsize, u32 ns)
{
	int ret = 0;
				...
				else if(0x2 == Conntype)
				{
					...
					else
					{
						...
						if(pAddStreamLinkCB)
						{
							...
							printf("%s 123 client ip: %s\n", __func__, inet_ntoa(clientaddr.sin_addr));
...
//	strcpy(lan_productnum, g_upnpid);
					...
					if (ret == 0)
					{
						if (sConfig.nIsPatrol == 1) //设备已经使能轮巡
						{
							...
							if (ret == 0)
							{
								...
								sPatrolPara.nPatrolChnNum = sConfig.nPatrolChnNum;
	...
	else if(type == CTRL_DEVICESEARCH_ACKCLIENT)//接收回应
	{
		...
		while(1)
		{
			...
			//deviceinfo.deviceIP = ntohl(deviceinfo.deviceIP);//因为对端没有调用htonl，
...
{
	static int ack = CTRL_DEVICESEARCH_ACKCLIENT;
	THREADHANDLE handle = 0;
	
	handle = IFly_CreateThread(CPAckSearchProc,NULL,PRI_CPCHECK,STKSIZE_CPCHECK,(u32)&ack,0,NULL);
