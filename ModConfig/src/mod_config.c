...
{
	//printf("%s not work, direct return\n", __func__);
	//return 0;
...
s32 ConfigLoadAllParaToMem(SConfigAll* para, s8* pFilePath)
{
	...
	para->psStrOsdPara = calloc(g_sConfigDvrProperty.nVidMainNum, sizeof(SModConfigStrOsdpara));
	para->psRectOsdPara = calloc(g_sConfigDvrProperty.nVidMainNum, sizeof(SModConfigRectOsdpara));
	...
	para->psVMotionPara 	= calloc(g_sConfigDvrProperty.nVidMainNum, sizeof(SModConfigVMotionParam));
	para->psVLostPara 		= calloc(g_sConfigDvrProperty.nVidMainNum, sizeof(SModConfigVLostParam));
	para->psVBlindPara		= calloc(g_sConfigDvrProperty.nVidMainNum, sizeof(SModConfigVBlindParam));
	...
	para->psVMotionSchPara 	= calloc(g_sConfigDvrProperty.nVidMainNum, sizeof(SModConfigVMotionSch));
	para->psPtzPara 		= calloc(g_sConfigDvrProperty.nVidMainNum, sizeof(SModConfigPtzParam));
	para->psIPCameraPara 	= calloc(g_sConfigDvrProperty.nVidMainNum, sizeof(SModConfigIPCameraParam));//NVR used
	...
	para->psSnapPara = calloc(g_sConfigDvrProperty.nVidMainNum, sizeof(SModConfigSnapChnPara));
...
s32 ConfigGetPreviewParam(SModConfigPreviewParam* para, s8* pFilepath)
{
	...
	//CONFIG_PREVIEW_PARAM_PatrolChnNum
	if(0 != IniGetValue(pFilepath, 
		CONFIG_PREVIEW_PARAM, 
		CONFIG_PREVIEW_PARAM_PatrolChnNum, 
		&tmp))
...
s32 ConfigGetParamFromMem(EMCONFIGPARATYPE emType, void* para, s32 nId, SConfigAll* pAllPara)
{
	...
	switch(emType)
	{
		...
		case EM_CONFIG_PARA_RECT_OSD:	
		case EM_CONFIG_PARA_PTZ:
			//printf("%d,Invalid para [%d]! nVidMainNum: %d\n", emType, nId, g_sConfigDvrProperty.nVidMainNum);
		case EM_CONFIG_PARA_VMOTION:
		case EM_CONFIG_PARA_VLOST:
		case EM_CONFIG_PARA_VBLIND:
		case EM_CONFIG_PARA_VMOTION_SCH:
		...
		for (i = 0; i < nCount; i++)
		{
			...
			else if(EM_CUSTOM_DEVICE_VIDEOSTAND == sCustomInfo.emType)
			{
				if (0 == CustomGetData(i, &sCustomData)) //保存视频制式
				{
					...
					if(sCustomData.nVideoStandard == EM_PAL || sCustomData.nVideoStandard == EM_NTSC)
					{
						...
						ModConfigSetParam(EM_CONFIG_PARA_SYSTEM, &sPIns, 0);
						//printf("客户定制视频制式成功!\n");
						printf("%s VideoStandard success!\n", __func__);
					}
			...
			else if(EM_CUSTOM_DEVICE_LANGUAGE == sCustomInfo.emType)
			{
				...
				if(EM_CUSTOM_ALL == emType)
				{
					...
					printf("file language [%d], Custom language[%d]\n",sPIns.nLangId,sCustomData.sLanguageParam.nLanguageIdDefault);
					...
					//printf("客户定制语言[%d]成功!\n",sPIns.nLangId);
					printf("%s Custom language success %d!\n", __func__, sPIns.nLangId);
				}
				else
				{
...
s32 ModConfigDefault(void)
{
	...
	memcpy(g_ConfigAllPara.psStrOsdPara, g_ConfigDefaultAllPara.psStrOsdPara, g_sConfigDvrProperty.nVidMainNum * sizeof(*g_ConfigAllPara.psStrOsdPara));
	memcpy(g_ConfigAllPara.psRectOsdPara, g_ConfigDefaultAllPara.psRectOsdPara, g_sConfigDvrProperty.nVidMainNum * sizeof(*g_ConfigAllPara.psRectOsdPara));
	...
	memcpy(g_ConfigAllPara.psVMotionPara, g_ConfigDefaultAllPara.psVMotionPara, g_sConfigDvrProperty.nVidMainNum * sizeof(*g_ConfigAllPara.psVMotionPara));
	memcpy(g_ConfigAllPara.psVLostPara, g_ConfigDefaultAllPara.psVLostPara, g_sConfigDvrProperty.nVidMainNum * sizeof(*g_ConfigAllPara.psVLostPara));
	memcpy(g_ConfigAllPara.psVBlindPara, g_ConfigDefaultAllPara.psVBlindPara, g_sConfigDvrProperty.nVidMainNum * sizeof(*g_ConfigAllPara.psVBlindPara));
	...
	memcpy(g_ConfigAllPara.psVMotionSchPara, g_ConfigDefaultAllPara.psVMotionSchPara, g_sConfigDvrProperty.nVidMainNum * sizeof(*g_ConfigAllPara.psVMotionSchPara));
	...
	memcpy(g_ConfigAllPara.psPtzPara, g_ConfigDefaultAllPara.psPtzPara, g_sConfigDvrProperty.nVidMainNum * sizeof(*g_ConfigAllPara.psPtzPara));
	...
	//NVR used
	memcpy(g_ConfigAllPara.psIPCameraPara, g_ConfigDefaultAllPara.psIPCameraPara, g_sConfigDvrProperty.nVidMainNum * sizeof(*g_ConfigAllPara.psIPCameraPara));
	
	memcpy(g_ConfigAllPara.psSnapPara, g_ConfigDefaultAllPara.psSnapPara, g_sConfigDvrProperty.nVidMainNum * sizeof(*g_ConfigAllPara.psSnapPara));
	//yaogang modify 20141209 深广平台参数
...
{
	if((para == NULL) || (nId < 0) || (nId >= g_sConfigDvrProperty.nVidMainNum) || (NULL == pFilepath))
	{
		printf("Invalid para! function: %s\n", __FUNCTION__);
		return -1;
	}
	...
	else if(strcasecmp(strvalue, "nvr") == 0)
	{
		para->protocol_type = PRO_TYPE_NVR;
	}
	else
	{
		para->protocol_type = PRO_TYPE_ONVIF;
...
s32 ConfigSetIPCameraParam(SModConfigIPCameraParam* para, s32 nId, s8* pFilepath)
{
	...
	else if(para->protocol_type == PRO_TYPE_NVR)
	{
		strcpy(strvalue, "nvr");
	}
	else
