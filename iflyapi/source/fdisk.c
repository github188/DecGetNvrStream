...
get_geometry(void)
{
	...
	printf("cylinders=%d,heads=%d,sectors=%d,sector_size=%d,sec_fac=%d,sector_offset=%d,total_number_of_sectors=%lld\n",cylinders,heads,sectors,sector_size,sec_fac,sector_offset,total_number_of_sectors);
...
int ifly_partition(char *path,int count,int fs)
{
	...
	uint cyls_per_pt = cylinders / count;
	for(i=0;i<count;i++)
	{
	...
	for(i=0; i<15; i++)
	{
		...
		for(j=0; j<count; j++)
		{
			...
			if(access(pt_name, F_OK) != 0)//?¡¤??2?¡ä??¨²¡ê?¡¤?????1¨°??¨¦?
			{
				break;
			}
		}
