...
setup_tables (void)
{
  ...
  if (sectors_per_cluster)
    ...
    printf("\nsectors_per_cluster=%d,cluster_size=%d,sector_size=%d\n\n\n",sectors_per_cluster,bs.cluster_size,sector_size);//wrchen 090909
  if (size_fat == 32)
    {
...
int fillindexfilehead(unsigned char *pbyBuf,int num)
{
	...
	p = (unsigned int *)(pbyBuf);
	*p = CT_LE_L(1);

	p = (unsigned int *)(pbyBuf+8);
	*p = CT_LE_L(1);

	p = (unsigned int *)(pbyBuf+16);
	*p = CT_LE_L(num);

	int i;
	for(i=0;i<=32;i++)
	{
		p = (unsigned int *)(pbyBuf+32+i*12);
		*p = CT_LE_L(0xffff);
	}
...
*/
  ...
  //根目录区包含的表项:nr_files个文件容器的长短文件表项、IFLYDVR短文件表项、
  ...
  unsigned int test_buf_entries = TEST_BUFFER_BLOCKS*BLOCK_SIZE/4;//3T4T
  ...
  for (x = 0; x < nr_fats; x++)
  {
	  for(y=0;y<nr_bufs;y++)
	  {
		  for(i=0;i<test_buf_entries;i++)
		  {
			  ...
			  // 2 : 前两个不用
			  if(pFat[i]-2-root_clusters-1-2*index_file_clusters - 2*pic_index_file_clusters > total_file_clusters)
			  {
				  pFat[i] = 0;
			  }
			  else if((pFat[i]-2-root_clusters-1-2*index_file_clusters - 2*pic_index_file_clusters)%file_clusters == 0)
			  {
				  pFat[i] = FAT_EOF;
			  }
		  }
  ...
  for(i=0;i<2;i++)
  {
	  ...
	  *(fat+2*sector_size+5) = 0xff;
	  ...
	  {
		  *((unsigned int *)(fat+2*sector_size)) = CT_LE_L(j);
