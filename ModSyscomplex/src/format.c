...
//#define PRINT_FORMAT_TIME
	...
	ret = ifly_partition(disk_name, count, 0xc);
	if(ret == -1)
	{
	...
	for(i=0; i<count; i++)
	{
		...
		if(ret == 1)
		{
			psHddInfo->total += (get_partition_total_space(&psHddInfo->ptn_index[i])/KB);
			
			if(i == 0)
			{
				char logFile[32] = {0};
				sprintf(logFile, "%s/log.hex", cmd);
				FILE* pFile = fopen(logFile,"wb+");
				if(pFile != NULL)
				{
					char loginfo[60];
	...
	psHddInfo->free = psHddInfo->total;
	...
	*pPrtnNum = 0;
	...
	if(phinfo->is_disk_exist)
	{
		...
		for(i=0;i<MAX_PARTITION_NUM;i++)
		{
			...
			{
				j++;
				phinfo->total += (long)(get_partition_total_space(&phinfo->ptn_index[i])/KB);
				phinfo->free += (long)(get_partition_free_space(&phinfo->ptn_index[i])/KB);
