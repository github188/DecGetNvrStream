		...
		switch(i)
		{
			...
			case 2://默认值
			{
				...
				if(0 == curID)//系统子页面
				{
					...
					{
						index = GetDWellSwitchChnNumDefault();
					}
					else	//NVR or 切换型解码器
					{
			...
			case 3://应用
			{
				...
				if(0 == curID) //系统子页面
				{
					...
					int curLanguage = 0;//pComboBox0[3]->GetCurSel();
					int curPatrolChnNumIndex = 0;//pComboBox0[3]->GetCurSel();
					int nRestart = 0;

					if (nNVROrDecoder == 2)//轮巡型解码器
					{
						curPatrolChnNumIndex = pComboBox0[3]->GetCurSel();
					}
					else
					{
						curLanguage = pComboBox0[3]->GetCurSel();
						if(curLanguage != m_lastLanguage)
						{
...
RESAVE:
					...
					if (nNVROrDecoder == 2)//轮巡型解码器
					{
						ret |= SetDWellSwitchChnNumIndex(pComboBox0[3]->GetCurSel());
					}
...
void CPageBasicConfigFrameWork::InitPage0()
{
		...
		if (i == 7)
		{
			if (nNVROrDecoder == 2)//轮巡型解码器
			{
				pStatic0[i] = CreateStatic(CRect(rtSubPage.left+tmpRt.left+3, rtSubPage.top+tmpRt.top+3, 
										rtSubPage.left+tmpRt.right-3,rtSubPage.top+tmpRt.bottom), 
										this, "&CfgPtn.DWellSwitchChnNum");
			}
			else //NVR or 切换型解码器
			{
				pStatic0[i] = CreateStatic(CRect(rtSubPage.left+tmpRt.left+3, rtSubPage.top+tmpRt.top+3, 
										rtSubPage.left+tmpRt.right-3,rtSubPage.top+tmpRt.bottom), 
										this, "&CfgPtn.Language");
			}
		}
		else
		{
	...
	//yaogang modify 20151203
	//轮巡通道数16-32-48-64
	SValue ChnNumList[10];
	int nChnNumReal = 0;
	GetDWellSwitchChnNumList(ChnNumList, &nChnNumReal, 10);
	...
	for(i=0; i<5; i++)
	{
		...
		switch(i+1)
		{
			...
			case 3:
			{
				if (nNVROrDecoder == 2)	//轮巡型解码器
				{
					for(int k=0; k<nChnNumReal; k++)
					{
						pComboBox0[i+1]->AddString(ChnNumList[k].strDisplay);
...
VD_BOOL CPageBasicConfigFrameWork::UpdateData( UDM mode )
{
	if(UDM_OPEN == mode)
	{
		...
		if (nNVROrDecoder == 2)	//轮巡型解码器
		{
			index = GetDWellSwitchChnNumIndex();
		}
		else	//NVR or 切换型解码器
		{
