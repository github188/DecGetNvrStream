...
//yaogang modify 2051121
	char tmp[20] = {0};
	GetProductNumber(tmp);
	if((0 == strcasecmp(tmp, "NR3564")))
	{
		//printf("%s yg ProductNumber: %s\n", __func__, tmp);
		nChSwitchBtn = 3;
	}
	printf("%s nChSwitchBtn: %d\n", __func__, nChSwitchBtn);
...
void CPageStartFrameWork::OnClkStartBtn()
{
	...
	ChMax = bizPara.nPatrolChnNum;//轮巡通道数
	if (ChMax > nChMax)
	{
		ChMax = nChMax;
	}
	...
	if(bFind)
	{
		switch(i)
		{
			...
			case START_TOOL2X2:
			{
                ...
                nCurPage %= ((ChMax%EM_BIZPREVIEW_4SPLITS)?(ChMax/EM_BIZPREVIEW_4SPLITS+1):(ChMax/EM_BIZPREVIEW_4SPLITS));
                ((CPageDesktop *)(this->GetParent()))->SetCurPreviewMode(previewMode);
                ((CPageDesktop *)(this->GetParent()))->SetCurPreModePage(nCurPage++);
			}break;
            //9画面预览
			case START_TOOL3X3:
			{
				...
				nCurPage %= ((ChMax%t)?(ChMax/t+1):(ChMax/t));
			...
			case START_TOOL4X4:
			{
				...
				{
					SetCurPreviewMode_CW(previewMode);//cw_preview
					SwitchPreview(previewMode, 0xff);//0xff模式不切换，通道切换
					nCurPage++;
				}
				else
				{
                ...
                nCurPage %= ((ChMax%EM_BIZPREVIEW_16SPLITS)?(ChMax/EM_BIZPREVIEW_16SPLITS+1):(ChMax/EM_BIZPREVIEW_16SPLITS));
                ((CPageDesktop *)(this->GetParent()))->SetCurPreviewMode(previewMode);
                ((CPageDesktop *)(this->GetParent()))->SetCurPreModePage(nCurPage);
			}break;
            //25画面预览
