...
, curID(0),chnNum(4)
{
	...
	//printf("%s this: %p, pBmp_button_normal: %p\n", __func__, this, pBmp_button_normal);
...
void CPageLiveConfigFrameWork::InitPage11()
{
	...
	SValue chList[64];//SValue chList[10];//csp modify
	int nChReal = 0;
...
void CPageLiveConfigFrameWork::InitPage20()
{
	...
	int nDwellReal;
	GetDwellTimeList(dwellList, &nDwellReal, 20);
	printf("CPageLiveConfigFrameWork::nDwellReal=%d\n",nDwellReal);
...
{	
	...
	if(maxchn > MASK_ROWS)
	{
		m_maxPage3 = maxchn%MASK_ROWS ? maxchn/MASK_ROWS+1 : maxchn/MASK_ROWS;
		//printf("%s 1 m_maxPage3: %d\n", __func__, m_maxPage3);
...
void CPageLiveConfigFrameWork::ShowSubPage( int mainID,int subID, BOOL bShow )
{
	...
	for (i=0; i<count; i++)
	{
		if(items[mainID].SubItems[subID][i])
		{
			...
			/*
			printf("%s mainID: %d, subID: %d, i: %d\n", \
				__func__, mainID, subID, i);
				*/
		}
	}
	...
	case XM_MOUSEMOVE:
		{
			...
			if(temp != last)
			{
					...
					|| temp == (pButton3[7]?(CButton*)pButton3[7]:(CButton*)!pButton3[7]))
				{
					...
					else if((0 == strcasecmp(tmp2, "NR3564")))
					{
						SetInfo("&CfgPtn.MaxMaskCif");
					}
