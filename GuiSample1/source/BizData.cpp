...
static int bRecord[64] = {0};
static int bMD[64] = {0};
static int bSensorAlarm[64] = {0};
static int bIPCExtAlarm[64] = {0};
static int bIPCCoverAlarm[64] = {0};
...
{
	//printf("EventPreviewFreshDeal 1\n");//csp modify 20150110
	if(!pBizPreviewData)
	{
	...
	switch(pBizPreviewData->emBizPreviewMode)
	{
		...
		case EM_BIZPREVIEW_PIP:
			...
			//return 0;
			break;
		case EM_BIZPREVIEW_HideItem: //隐藏控件
			//yaogang modify 20151202
			pDeskTop->ShowFrameSplitLine(FALSE);
			pAlarmList->Close();
			printf("%s EM_BIZPREVIEW_HideItem\n", __func__);
			return 0;
		case EM_BIZPREVIEW_CLOSE:
			//yaogang modify 20151202
			//pDeskTop->ShowFrameSplitLine(FALSE);
			pAlarmList->Close();
			printf("%s EM_BIZPREVIEW_CLOSE, ignore param\n", __func__);
			return 0;
		default:
	...
	//yaogang modify 20151202
	pDeskTop->ShowFrameSplitLine(TRUE);
	...
	//csp modify 20150106
	nChnWidth = (nChnWidth / 2) * 2;
	nChnHeight = (nChnHeight / 2) * 2;
	...
	for(i = nStartChn; i < nMaxChn; i++)
	{
		...
		/*码流*/
		char chKbps[12];
		int StreamKbps = Get_StreamKbps(i)+Get_StreamKbps(i+nMainChn);
...
//yaogang modify 20141119 码流显示残像，在双击第16通道时
	...
	for(i = nStartChn; i < nMaxChn; i++)
	{
		...
		else
		{
			...
			StreamKbps = Get_StreamKbps(i)+Get_StreamKbps(i+nMainChn);
		}
...
int BizEventCB(SBizEventPara* pSBizEventPara)
{
	...
	switch(pSBizEventPara->emType)
	{
		...
		//yaogang modify 20160201
		//nvr.cpp 中远程NVR 报警
		case EM_BIZ_EVENT_ALARM_RemoteNVR_VLOST:
		{
			//printf("pSBizEventPara->emType = %d\n", pSBizEventPara->emType);
			if(pSBizEventPara->sBizAlaStatus.nCurStatus)//视频丢失添加信息显示
			{
				AlarmListAdd(pSBizEventPara->sBizAlaStatus.nChn, EM_ALRM_ALARM_RemoteNVR_VLOST, 1, pSBizEventPara->sBizAlaStatus.nvrIP);				
			}
			else
			{
				AlarmListDelete(pSBizEventPara->sBizAlaStatus.nChn, EM_ALRM_ALARM_RemoteNVR_VLOST, 0, pSBizEventPara->sBizAlaStatus.nvrIP);
				//报警解除EM_DVR_ALARM_RESUME AlarmRemmove				
			}
			
		}break;
		case EM_BIZ_EVENT_ALARM_RemoteNVR_DISK_ERR:
		{
			//printf("pSBizEventPara->emType = %d\n", pSBizEventPara->emType);
			if(pSBizEventPara->sBizAlaStatus.nCurStatus)
			{
				AlarmListAdd(pSBizEventPara->sBizAlaStatus.nChn, EM_ALRM_ALARM_RemoteNVR_DISK_ERR, 1, pSBizEventPara->sBizAlaStatus.nvrIP);				
			}
			else
			{
				AlarmListDelete(pSBizEventPara->sBizAlaStatus.nChn, EM_ALRM_ALARM_RemoteNVR_DISK_ERR, 0, pSBizEventPara->sBizAlaStatus.nvrIP);
				//报警解除EM_DVR_ALARM_RESUME AlarmRemmove				
			}
		}break;
		case EM_BIZ_EVENT_ALARM_RemoteNVR_DISK_LOST:
		{
			//printf("pSBizEventPara->emType = %d\n", pSBizEventPara->emType);
			if(pSBizEventPara->sBizAlaStatus.nCurStatus)
			{
				AlarmListAdd(pSBizEventPara->sBizAlaStatus.nChn, EM_ALRM_ALARM_RemoteNVR_DISK_LOST, 1, pSBizEventPara->sBizAlaStatus.nvrIP);				
			}
			else
			{
				AlarmListDelete(pSBizEventPara->sBizAlaStatus.nChn, EM_ALRM_ALARM_RemoteNVR_DISK_LOST, 0, pSBizEventPara->sBizAlaStatus.nvrIP);
				//报警解除EM_DVR_ALARM_RESUME AlarmRemmove				
			}
		}break;
		case EM_BIZ_EVENT_ALARM_VLOSS:
			...
			EventVLossAlarmDeal(&pSBizEventPara->sBizAlaStatus);
			break;
		case EM_BIZ_EVENT_ALARM_VBLIND:
		...
		case EM_BIZ_EVENT_UPGRADE_RUN:
			{
				...
				if(g_pPages)
				{
					...
					if ((CPageUpdate*)g_pPages[EM_PAGE_UPDATE])
...
void GetVideoFormatList( SValue* psValueList, int* nRealNum, int nMaxNum)
{
	...
	if (psValueList)
	{
		...
		*nRealNum = real;
		...
		if (*nRealNum > nMaxNum)
		{
			printf("ERROR*************************** %s, MaxChnNum:%d > nMaxNum:%d\n", \
				__func__, *nRealNum, nMaxNum);
...
void GetLanguageList( SValue* psValueList, int* nRealNum, int nMaxNum)
{
	...
	if(psValueList)
	{
		...
		ret = BizConfigGetParaStr(EM_BIZ_CFG_PARALIST_LANGSUP, (s8**)g_strList, (u8*)&real, 20, 64);
		*nRealNum = real;
...
void GetDWellSwitchChnNumList( SValue* psValueList, int* nRealNum, int nMaxNum)
{
	int i = 0;
	int nDWellChnNum[4] = {16, 32, 48, 64};
	...
	if(psValueList)
	{
		...
		*nRealNum = 4;
		...
		for(i=0; i<*nRealNum; i++)
		{
			psValueList[i].nValue = nDWellChnNum[i];
			sprintf(psValueList[i].strDisplay, "%d", nDWellChnNum[i]);
		}
	}
...
int GetDWellSwitchChnNumIndex()
{
    ...
    switch(bizPara.nPatrolChnNum)
    {
        ...
        default:
        {
			printf("Error *********** %s, nPatrolChnNum: %d invalid\n", __func__, bizPara.nPatrolChnNum);
            ret = 3;
        } break;
    }
...
int SetDWellSwitchChnNumIndex(int index)
{
	SBizParaTarget bizTar;
	bizTar.emBizParaType = EM_BIZ_PATROLPARA;
	SBizCfgPatrol bizPara;
    ...
    switch(index)
    {
        case 0:
        {
            bizPara.nPatrolChnNum = 16;
        } break;
        case 1:
        {
            bizPara.nPatrolChnNum = 32;
        } break;
        case 2:
        {
            bizPara.nPatrolChnNum = 48;
        } break;
        case 3:
        {
            bizPara.nPatrolChnNum = 64;
        } break;
        default:
        {
			printf("Error *********** %s, index: %d invalid\n", __func__, index);
            bizPara.nPatrolChnNum = 64;
        } break;
    }
...
//yaogang modify 20151203
int GetDWellSwitchChnNumDefault()
{
	SBizParaTarget bizTar;
	bizTar.emBizParaType = EM_BIZ_PATROLPARA;
	SBizCfgPatrol bizPara;
    ...
    switch(bizPara.nPatrolChnNum)
    {
        ...
        default:
        {
			printf("Error *********** %s, PatrolChnNum: %d invalid\n", __func__, bizPara.nPatrolChnNum);
            ret = 3;
        } break;
    }
...
int GetScreenStrSelCOMBLIST(u32 nItemID, SCombList* pList)
{
	...
	printf("GetScreenStrSelCOMBLIST ID %d\n", nItemID);
...
{
	u32 nvrIP;
	SAlarmStatus m_byVLoss[32];//在此限定远程NVR最大支持32路IPC的
	SAlarmStatus m_byDiskErr[MAX_HDD_NUM];
	SAlarmStatus m_byDiskLost[MAX_HDD_NUM];
} SRemoteNVRStatus;//MAX_REMOTE_NVR
...
public:
	...
	SAlarmStatus m_byVLoss[CHN_MAX];
	SAlarmStatus m_byVMotion[CHN_MAX];
	SAlarmStatus m_bySensor[72];
	SAlarmStatus m_byIPCExtSensor[CHN_MAX];
	SAlarmStatus m_by485ExtSensor[CHN_MAX];
	SAlarmStatus m_byIPCCover[CHN_MAX];
	SAlarmStatus m_byDiskLost[MAX_HDD_NUM];
	SAlarmStatus m_byDiskErr[MAX_HDD_NUM];
	SAlarmStatus m_byDiskNone;
	SRemoteNVRStatus m_byNVR[MAX_REMOTE_NVR];//MAX_REMOTE_NVR
	/*
	...
	for (j=0; j<MAX_REMOTE_NVR; j++)
	{
		...
		if (pnvr->nvrIP != 0)
		{
			...
			{		
				...
				else if (pnvr->m_byDiskLost[i].still_stamp)
				{
					return;
				}
			}
			...
			{
				//该远程NVR的报警都恢复了--- 清除
				memset(pnvr, 0, sizeof(SRemoteNVRStatus));
			}
		}	
	}
...
void AlarmListAdd(u8 chanel, ALARMLISTTYPE cmd, u8 status, const u32 nvrIP)
{
	int i = 0, index = 0;
	int first_valid = -1;
	...
	//yaogang modify 20160201
	//nvr.cpp 中远程NVR 报警
	else if(cmd == EM_ALRM_ALARM_RemoteNVR_VLOST)
	{
	#if 0
		struct in_addr in;
		in.s_addr = nvrIP;
		
		printf("%s nvr alarm VLOST, IP: %s, chn: %u, status: %u\n", 
			__func__, inet_ntoa(in), chanel, status);
	#endif
		first_valid = -1;
		for (i=0; i<MAX_REMOTE_NVR; i++)
		{
			if (g_cAlarmListObj.m_byNVR[i].nvrIP == 0)
			{
				first_valid = i;				
				break;
			}
			else if (nvrIP == g_cAlarmListObj.m_byNVR[i].nvrIP)
			{
				break;
			}
		}
		...
		else
		{
			if (-1 != first_valid)
			{
				index = first_valid;//找到
			}
			else
			{
				printf("%s ***Error*** 1 cmd: %u, MAX_REMOTE_NVR all in use\n", __func__, cmd);
				g_cAlarmListObj.m_Mutex.Leave();
				return;
			}
		}

		g_cAlarmListObj.m_byNVR[index].nvrIP = nvrIP;
		
		if(!g_cAlarmListObj.m_byNVR[index].m_byVLoss[chanel].current)
		{
			if (!g_cAlarmListObj.m_byNVR[index].m_byVLoss[chanel].still_stamp)
			{
				g_cAlarmListObj.m_bChanged = TRUE;
				g_cAlarmListObj.m_byNVR[index].m_byVLoss[chanel].still_stamp = 1;
			}
		}
		
		g_cAlarmListObj.m_byNVR[index].m_byVLoss[chanel].current = 1;
		
	}
	else if(cmd == EM_ALRM_ALARM_RemoteNVR_DISK_ERR)
	{
		first_valid = -1;
		for (i=0; i<MAX_REMOTE_NVR; i++)
		{
		...
		else
		{
			...
			{
				printf("%s ***Error*** 2 cmd: %u, MAX_REMOTE_NVR all in use\n", __func__, cmd);
				g_cAlarmListObj.m_Mutex.Leave();
				return;
			}
		}
		...
		if(!g_cAlarmListObj.m_byNVR[index].m_byDiskErr[chanel].current)
		{
			if (!g_cAlarmListObj.m_byNVR[index].m_byDiskErr[chanel].still_stamp)
			{
				g_cAlarmListObj.m_bChanged = TRUE;
				g_cAlarmListObj.m_byNVR[index].m_byDiskErr[chanel].still_stamp = 1;
			}
		}
		
		g_cAlarmListObj.m_byNVR[index].m_byDiskErr[chanel].current= 1;
	}	
	else if(cmd == EM_ALRM_ALARM_RemoteNVR_DISK_LOST)
	{
		first_valid = -1;
		for (i=0; i<MAX_REMOTE_NVR; i++)
		{
		...
		else
		{
			...
			{
				printf("%s ***Error*** 3 cmd: %u, MAX_REMOTE_NVR all in use\n", __func__, cmd);
				g_cAlarmListObj.m_Mutex.Leave();
				return;
			}
		}
		...
		if(!g_cAlarmListObj.m_byNVR[index].m_byDiskLost[chanel].current)
		{
			if (!g_cAlarmListObj.m_byNVR[index].m_byDiskLost[chanel].still_stamp)
			{
				g_cAlarmListObj.m_bChanged = TRUE;
				g_cAlarmListObj.m_byNVR[index].m_byDiskLost[chanel].still_stamp = 1;
			}
		}
		
		g_cAlarmListObj.m_byNVR[index].m_byDiskLost[chanel].current= 1;
	}	
	else
	{
		printf("%s ***Error*** AlarmListAdd alarm source unknow\n", __func__);
...
void AlarmListDelete(u8 chanel,ALARMLISTTYPE cmd,u8 status, const u32 nvrIP)
{
	int i = 0;
	//int first_valid = -1;
	...
	else if(cmd == EM_ALRM_ALARM_RemoteNVR_VLOST)
	{
		...
		for (i=0; i<MAX_REMOTE_NVR; i++)
		{
			if (nvrIP == g_cAlarmListObj.m_byNVR[i].nvrIP)
			{
				break;
			}
		}
		...
		{
			//找到已有
			if(g_cAlarmListObj.m_byNVR[i].m_byVLoss[chanel].still_stamp)
			{				
				g_cAlarmListObj.m_bChanged = TRUE;
				g_cAlarmListObj.m_byNVR[i].m_byVLoss[chanel].still_stamp = 0;				
			}
			
			g_cAlarmListObj.m_byNVR[i].m_byVLoss[chanel].current = 0;
		...
		{
			printf("%s ***Error*** cmd: %u, m_byNVR[%d] not found\n", 
				__func__, cmd, MAX_REMOTE_NVR);
	...
	else if(cmd == EM_ALRM_ALARM_RemoteNVR_DISK_ERR)
	{
		...
		{
			//找到已有			
			g_cAlarmListObj.m_byNVR[i].m_byDiskErr[chanel].current= 0;
		}
		else
		{
			printf("%s ***Error*** cmd: %u, m_byNVR[%d] not found\n", 
	...
	else if(cmd == EM_ALRM_ALARM_RemoteNVR_DISK_LOST)
	{
		...
		{
			//找到已有			
			g_cAlarmListObj.m_byNVR[i].m_byDiskLost[chanel].current= 0;
		}
		else
		{
			printf("%s ***Error*** cmd: %u, m_byNVR[%d] not found\n", 
...
int RefreshAllStatus()
{
	return g_cAlarmListObj.RefreshAllStatus();
}
...
//用户已经查看了报警信息
//窗口关闭前去除报警已经恢复的源信息
int CAlarmList::RefreshAllStatus()
{
	int i = 0, j = 0;
	...
	for(i = 0; i < GetVideoMainNum(); i++)
	{
		...
		{
			printf("%s m_byIPCCover[%d].current: %d\n", __func__, i, m_byIPCCover[i].current);
	...
	for (j=0; j<MAX_REMOTE_NVR; j++)
	{
		...
		if (pnvr->nvrIP != 0)
		{
			...
			for (i=0; i<32; i++)//在此限定远程NVR最大支持32路IPC的
			{
				if(pnvr->m_byVLoss[i].still_stamp)// 曾经触发过
				{
					if (pnvr->m_byVLoss[i].current == 0)//现在已经恢复
					{
						m_bChanged = TRUE;
						pnvr->m_byVLoss[i].still_stamp = 0;
					}
					else	//现在未恢复
					{
						bAlarmIconShow = TRUE;
			...
			for (i=0; i<MAX_HDD_NUM; i++)//在此限定远程NVR最大支持32路IPC的
			{
				if(pnvr->m_byDiskErr[i].still_stamp)// 曾经触发过
				{
					if (pnvr->m_byDiskErr[i].current == 0)//现在已经恢复
					{
						m_bChanged = TRUE;
						pnvr->m_byDiskErr[i].still_stamp = 0;
					}
					else	//现在未恢复
					{
						bAlarmIconShow = TRUE;
				...
				if(pnvr->m_byDiskLost[i].still_stamp)// 曾经触发过
				{
					if (pnvr->m_byDiskLost[i].current == 0)//现在已经恢复
					{
						m_bChanged = TRUE;
						pnvr->m_byDiskLost[i].still_stamp = 0;
					}
					else	//现在未恢复
					{
						bAlarmIconShow = TRUE;
	...
	return bAlarmIconShow;
}

//yaogang modify 20160201
//清除所有状态
void CAlarmList::ClrAllStatus()
{
	m_Mutex.Enter();
	...
	memset(m_byNVR, 0, sizeof(m_byNVR));
...
void CAlarmList::RefreshAlarmList(u8 page)
{
	...
	if(m_bChanged)
	{
		...
		u8 NVR_disklostflag = 0;
		SAlarmListItem NVR_disklostitem;
		struct in_addr in;
		char ipstr[32] = {0};		
			...
			if (pnvr->nvrIP != 0)
			{
				...
				{					
					if(pnvr->m_byVLoss[i].still_stamp)
					{
						...
						{
							sprintf(achStr, "%s: %d", inet_ntoa(in), i+1);
						}
				...
				//硬盘丢失
				NVR_disklostflag = 0;
				NVR_disklostitem.type = ipstr;
				NVR_disklostitem.type += GetParsedString("&CfgPtn.DiskLost");
				NVR_disklostitem.detail = "";
				...
				{		
					...
					if(pnvr->m_byDiskLost[i].still_stamp)
					{
						char achStr[32] = {0};
						if(NVR_disklostflag)
						{
						...
						NVR_disklostitem.detail += achStr;
						NVR_disklostflag = 1;
					}
				}
				if(NVR_diskerrflag)
				...
				if(NVR_disklostflag)
				{
					m_vCurrItems.push_back(NVR_disklostitem);
				}
			}
		}
		//
