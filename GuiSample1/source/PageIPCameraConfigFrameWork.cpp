...
void CPageIPCameraConfigFrameWork::InitPage0()
{
	...
	rt1.left = rt1.right+4;
	rt1.right = rt1.left+80;
	pButton0[button_index] = CreateButton(rt1, this, "&CfgPtn.AllDelete", (CTRLPROC)&CPageIPCameraConfigFrameWork::OnClickSubPage, NULL, 0, FALSE);
	pButton0[button_index]->SetBitmap(pBmp_button_normal, pBmp_button_select, pBmp_button_push, pBmp_button_push);
	button_index++;
	...
	case 6://自动分配
		{
			...
			unsigned int pro_type = (unsigned int)(-1);
...
VD_BOOL CPageIPCameraConfigFrameWork::UpdateData(UDM mode)
{
	if(UDM_OPEN == mode)
	{
		...
		/*
		pButton0[16]->Show(FALSE);//应用
		//pButton0[17]->Show(FALSE);//退出
		pButton0[18]->Show(FALSE);//上一页
		pButton0[19]->Show(FALSE);//下一页
		*/
		pButton0[17]->Show(FALSE);//应用
		//pButton0[18]->Show(FALSE);//退出
		pButton0[19]->Show(FALSE);//上一页
		pButton0[20]->Show(FALSE);//下一页
	}
