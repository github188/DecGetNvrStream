...
#endif
{
	...
	if(m_screen == 0)
	{
		...
		else
		{
			...
			printf("%s nOffsetXCvbs: %d, nOffsetYCvbs: %d, bottom: %d, right: %d\n", \
				__func__, nOffsetXCvbs, nOffsetYCvbs, \
				m_RectScreen.bottom, m_RectScreen.right);
	...
	InitChnState(maxChn, EM_STATE_MAX);

	//InitRegion(maxChn);

	//add by liu
	InitChnKbps(maxChn);
	InitChnElseState(maxChn, EM_ELSE_STATE_MAX);
	...
	case 64:
		emCurPreviewMode = EM_BIZPREVIEW_16SPLITS;
		eMaxViewMode = EM_BIZPREVIEW_64SPLITS;
		RowNum = ColNum = 4;
		break;
	default:
		emCurPreviewMode = EM_BIZPREVIEW_4SPLITS;
	...
	if (maxChn == 64)
	{
		emLastPreMode = emCurPreviewMode;
	}
...
*/			
		...
		//printf("yg emCurPreviewMode: %d\n", emCurPreviewMode);
	        if(EM_BIZPREVIEW_1SPLIT != emCurPreviewMode)
	        {
				...
				{
					printf("yg eMaxViewMode: %d, nPreviewPage: %d\n", eMaxViewMode, nPreviewPage);
					switch(eMaxViewMode)
					{
...
void CPageDesktop::ShowFrameSplitLine(BOOL bShow)
{
	int i = 0;
	for (i=0; i<m_vFrameSplitLine.size(); i++)
	{
		m_vFrameSplitLine[i]->Show(bShow);
	}
}
...
void CPageDesktop::SetChnElseStateBMP(int nChn, int nStatetype, VD_PCSTR pszBmpName1, VD_PCSTR pszBmpName2 )
{
	...
	int nIdx = (m_nMaxElseStatetype*nChn + nStatetype)*2; //每个图标都有两种状态
...
{
    nPreviewPage = nPage;
	printf("%s nPreviewPage: %d\n", __func__, nPreviewPage);
}
...
u32 CPageDesktop::ClikCurChn(EMBIZPREVIEWMODE emMod, u32 nPage, int x, int y)
{
	...
	GetVgaResolution(&nScreanWidth, &nScreanHeight);
	//yaogang modify 20151204
	int ChMax = GetMaxChnNum();
	SBizParaTarget bizTar;
	bizTar.emBizParaType = EM_BIZ_PATROLPARA;
	SBizCfgPatrol bizPara;
	
	if(BizGetPara(&bizTar, &bizPara))
	{
		printf("%s BizGetPara EM_BIZ_PATROLPARA failed\n", __func__);
		return 255;
	}
	//轮巡通道数
	if (ChMax > bizPara.nPatrolChnNum)
	{
		ChMax = bizPara.nPatrolChnNum;
	}
	...
	//printf("%s ChMax: %d, nPreWinPage: %d, nPage: %d\n", \
		//__func__, ChMax, nPreWinPage, nPage);
    for(u32 i=0; i<RowNum; ++i)
    {
        for(u32 j=0; j<ColNum; ++j)
        {
            ...
            {
                //最后一页时要全部填充完
                if(nPage == ChMax/nPreWinPage && nPage != 0)
                {
					//printf("%s ChMax: %d, nPreWinPage: %d, nPage: %d, (%d, %d)\n", \
							//__func__, ChMax, nPreWinPage, nPage, i, j);
                    return nPreWinPage*nPage+i*ColNum+j-(nPreWinPage*(nPage+1)-ChMax);
                } 
                else 
                {
