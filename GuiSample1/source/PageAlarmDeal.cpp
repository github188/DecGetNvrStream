...
void CPageAlarmDeal::OnClickSubPage()
{
	...
	{	
		...
		else
		{
			...
			{
				//触发报警
				int nRealChNum = sizeof(pChkTriggerAlarmPG0)/sizeof(pChkTriggerAlarmPG0[0]);
...
#if 1
	if (nNVROrDecoder == 1)//跃天: 1 nvr，2 轮巡解码器，3 切换解码器
	{
...
VD_BOOL CPageAlarmDeal::UpdateData( UDM mode )
{
	...
	if( UDM_OPEN == mode)
	{
		...
		//nRealChNum = sizeof(pChkTriggerChnPG1)/sizeof(pChkTriggerChnPG1[0]);
		nRealChNum = GetMaxChnNum();
		//printf("%s 3 nRealChNum: %d\n", __func__, nRealChNum);
		
		int IsRecStatue[8] = {0};
		memset(IsRecStatue, 0xff, sizeof(IsRecStatue));
...
if (nParentLabel != 4)
{
	...
	rt.right = rt.left + 24;
	rt.bottom = rt.top + 24;
	for (i = 0; i<sizeof(pChkTriggerAlarmPG0)/sizeof(pChkTriggerAlarmPG0[0]); i++)
	{
...
{	
	int MaxChnNum = GetMaxChnNum();
	//int rows = GetMaxChnNum()%8?GetMaxChnNum()/8+1:GetMaxChnNum()/8;
	int rows = (MaxChnNum+7)/8;
#if 1
	pChkTriggerChnPG1 = (CCheckBox**)calloc(MaxChnNum, sizeof(CCheckBox*));
	memset(pChkTriggerChnPG1,0,sizeof(pChkTriggerChnPG1));
	pChkTriggerAllPG1 = (CCheckBox**)calloc(rows, sizeof(CCheckBox*));
