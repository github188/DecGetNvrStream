...
void CPageBasicConfigFrameWork::InitPage0()
{
	...
	for(i=0; i<4; i++)
	{
		if (i ==3)
		{
			pTable0->GetTableRect(0, 2, &tmpRt);
		...
		if (i == 0)//显示向导
		{
			pTable0->GetTableRect(1,0,&tmpRt);//上移一行
		...
		if(i == 1)
		{
			...
			pComboBox01[0] = CreateComboBox(CRect(rtSubPage.left+tmpRt.left+2, rtSubPage.top+tmpRt.top+1, 
											rtSubPage.left+tmpRt.right-15,rtSubPage.top+tmpRt.bottom-1), 
											this, NULL, NULL, NULL, 0);
			pComboBox01[0]->SetBkColor(VD_RGB(67,77,87));
			...
			{
				pComboBox01[0]->AddString(LockTimeList[k].strDisplay);
				//printf("InitPage0 AddString : %s\n",LockTimeList[k].strDisplay);
			}
			
			pComboBox01[0]->SetCurSel(0);
			pComboBox01[0]->Show(FALSE);
			
			items[2].push_back(pComboBox01[0]);
			
		}
		if (i == 3)  //显示实时码流
		{
			pTable0->GetTableRect(1,2,&tmpRt);//上移一行
	...
	int item_num = sizeof(statictext)/sizeof(statictext[0]);
	...
	SValue dWellSwitchPicture[5];//轮巡画面选择: 单画面，四画面，九画面
	int nPicturesReal;
	GetDWellSwitchPictureList(dWellSwitchPicture, &nPicturesReal, 5);
	/*
