...
//#ifdef ENC_SHOW_SMALL_FONT
	...
	if (m_FileFontSmall.Open(FONT_BIN"/FontSmallEn.bin", CFile::modeRead))
	{
	  	if (m_FileFontSmall.Read(&m_UFHSmall, sizeof(UCS_FONT_HEADER)) == sizeof(UCS_FONT_HEADER))
	  	{
			if (m_UFHSmall.size == m_FileFontSmall.GetLength())
			{
				if (m_UFHSmall.blocks)
				{
			...
			{
				printf("%s m_UFHSmall.size: %d, m_FileFontSmall.GetLength(): %d\n", __func__, m_UFHSmall.size, m_FileFontSmall.GetLength());
			}
	  	}
		else
		{
			printf("%s m_FileFontSmall.Read(&m_UFHSmall, sizeof(UCS_FONT_HEADER)): %d, sizeof(UCS_FONT_HEADER): %d\n", \
				__func__, m_FileFontSmall.Read(&m_UFHSmall, sizeof(UCS_FONT_HEADER)), sizeof(UCS_FONT_HEADER));
		}
	}
	else
	{
