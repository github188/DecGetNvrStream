...
void CChallenger::Initialize(int argc, char * argv[])
{
	...
	{
		if (DvrInfo.nNVROrDecoder != 1) //解码器版本
		{
			//yaogang modify 20150324
			sBizInitPara.nModMask &= ~(MASK_BIZ_MOD_RECORD|MASK_BIZ_MOD_PLAYBACK|MASK_BIZ_MOD_SNAP|MASK_BIZ_MOD_ALARM);
		}
	}
	...
	if(0 == strcasecmp(DvrInfo.sproductnumber, "NR3564"))
	{
		sBPP.emBizPreviewMode = EM_BIZPREVIEW_16SPLITS;
	}
	...
	if (DvrInfo.nNVROrDecoder == 1) //NVR
	{
...
*/
	...
	if(GetShowGuide())
    {
		...
		if((0 == strcasecmp(tmp, "NR3132")) \
			|| (0 == strcasecmp(tmp, "NR3124")) \
			|| (0 == strcasecmp(tmp, "NR3564")))
		{
	...
	if(!GetShowGuide())
	{
		...
		{
			pthread_t pid_tmp = 0;
			pthread_create(&pid_tmp, NULL, HideHddInfo, NULL);
		}
	}
	#endif
	...
	for(int idx = 0; idx < nChMax; idx++)
	{
		...
		//ipcam.bStreamRequest[1] = 1;//默认仅开启子码流
