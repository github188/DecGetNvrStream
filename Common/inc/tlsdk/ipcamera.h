...
typedef struct ipc_unit
{
	...
	char bStreamRequest[2];//是否请求主/次码流
	//主码流音频开关,未使用
	//NVR的最大通道数
	volatile char main_audio_sw;
	//次码流音频开关,未使用
	//要申请NVR 的哪一路码流
	volatile char sub_audio_sw;
	char reserved[43];
...
typedef enum
{
	...
	REAL_STREAM_STATE_ALARM,
}real_stream_state_e;

typedef int (*RealStreamCB)(real_stream_s *stream, unsigned int dwContext);
typedef int (*StreamStateCB)(int chn, real_stream_state_e msg, unsigned int nvrIP, unsigned int alarm_key);
