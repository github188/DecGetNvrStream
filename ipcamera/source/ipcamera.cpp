...
{
	//printf("cam1->enable(%d), cam2->enable(%d)\n", cam1->enable, cam2->enable);
	...
	{
		if (cam1->protocol_type == PRO_TYPE_NVR)
		{
			//要申请NVR 的哪一路码流不相同
			if (cam1->sub_audio_sw != cam2->sub_audio_sw)
			{
				return 0;
			}
		}
...
void *videoConnFxn(void *arg)
{
	...
	while(g_init_flag)
	{
		...
		for(i = 0; i < (int)g_chn_count; i++)
		{
			...
			//yaogang modify
			else if (memcmp(g_chn_info[i].newcam.bStreamRequest, \
							g_chn_info[i].cam.bStreamRequest, \
							sizeof(g_chn_info[i].newcam.bStreamRequest)))
			{
				//码流请求发生改变
				reconn = 1;
				g_chn_info[i].cam = ipcam;
				printf("videoConnFxn, chn%d stream request changed......\n", i);
			}
		...
		for(i = 0; i < (int)g_chn_count; i++)
		{
			ipcam = &g_chn_info[i].cam;
			
			int j = 0;
			for(j = 0; j < 2; j++)
			{
				channel = i + (j * g_chn_count);
				byCurStreamOpen = 0;
				...
				if(!byCurStreamOpen)//ipcam->enable == 0
				{
					//yaogang modify
					//g_chn_info[i].cam.bStreamRequest[j] = 0;
					//
				...
				{
					//printf("yg 1 channel:%d\n", channel);
					if (g_chn_info[i].cam.bStreamRequest[j])//使能请求码流
					{
						if(!IPC_GetLinkStatus(channel))//当前未连接
						{
							//printf("yg 2 channel:%d\n", channel);
							g_stream_info[channel].link_failed_count++;
							
							if(g_stream_info[channel].link_failed_count > 20)
							{
								//printf("yg 4 channel:%d\n", channel);
								//g_stream_info[channel].link_failed_count = 0;
					...
					{
						g_stream_info[channel].link_failed_count = 0;
						if(IPC_GetLinkStatus(channel))//当前已连接则关闭
						{
							//printf("chn%d stop-4, bStreamRequest = 0\n", channel);
							IPC_Stop(channel);
						}
					}
				}
...
int IPC_Init(unsigned int chn_num)
{
	...
	g_chn_count = chn_num;
	printf("%s g_chn_count: %d\n", __func__, g_chn_count);
	
	g_chn_info = (ipc_chn_info *)malloc(g_chn_count*sizeof(ipc_chn_info));
	if(g_chn_info == NULL)
	{
		printf("%s g_chn_info malloc %d failed\n", __func__, g_chn_count);
		return -1;
	}
	...
	if(g_stream_info == NULL)
	{
		free(g_chn_info);
		g_chn_info = NULL;
		printf("%s g_stream_info malloc %d failed\n", __func__, g_chn_count*2);
		return -1;
	}
...
//yaogang modify 20140918
	...
	if(g_stream_cnt == NULL)
	{
		...
		free(g_chn_info);
		g_chn_info = NULL;
		printf("%s g_stream_cnt malloc %d failed\n", __func__, g_chn_count*2);
		return -1;
	}
...
int IPC_Set(int chn, ipc_unit *ipcam)
{
	...
	*/
	...
	{
		/*
		g_chn_info[chn].ops.Open = NVR_CMD_Open;
		g_chn_info[chn].ops.Close = NVR_CMD_Close;
		g_chn_info[chn].ops.SetImageParam = NVR_CMD_SetImageParam;
		g_chn_info[chn].ops.GetImageParam = NVR_CMD_GetImageParam;
		g_chn_info[chn].ops.PtzCtrl = NVR_CMD_PtzCtrl;
		g_chn_info[chn].ops.SetTime = NVR_CMD_SetTime;
		g_chn_info[chn].ops.SetMD = NULL;
		g_chn_info[chn].ops.Reboot = NVR_CMD_Reboot;
		g_chn_info[chn].ops.RequestIFrame = NVR_CMD_RequestIFrame;
		*/
	}
...
int DoStreamStateCallBack(int chn, real_stream_state_e msg, u32 nvrIP, u32 alarm_key)
{
	//ipc_unit  ipcam;
	
	if(chn < 0 || chn >= (int)(g_chn_count*2))
	{
		printf("DoStreamStateCallBack: param error, chn%d\n", chn);
		return -1;
	}
	...
	if(g_pStateCB != NULL)
	{
		...
		(g_pStateCB)(chn, msg, nvrIP, alarm_key);
	}
...
int DoRealStreamCallBack(real_stream_s *stream, unsigned int dwContext)
{
	...
	if(chn < 0 || chn >= (int)(g_chn_count*2))
	{
		printf("DoRealStreamCallBack: param error, chn%d, g_chn_count: %d\n", chn, g_chn_count);
		return -1;
	}
...
int IPC_Start(int chn)
{
	...
	memset(&g_stream_info[chn].ops, 0, sizeof(g_stream_info[chn].ops));	
	...
	*/
	...
	{
		g_stream_info[chn].ops.Start = NVR_Start;
		g_stream_info[chn].ops.Stop = NVR_Stop;
		g_stream_info[chn].ops.GetLinkStatus = NVR_GetLinkStatus;
		
		//IPC_CMD_SetTime(chn, 0, 1);
		
		ret = NVR_Start(chn, DoRealStreamCallBack, chn, NULL, ipcam->dwIp, ipcam->wPort, ipcam->user, ipcam->pwd, (u8)ipcam->sub_audio_sw);
		if (ret)
		{
			printf("%s chn%d NVR_Start failed\n", __func__, chn);
		}
	}
	/*
	else if(ipcam->protocol_type == PRO_TYPE_FULLHAN)
	...
	*/
	...
	if(ret == 0)
	{
		if(g_pStateCB != NULL)
		{
			...
			(g_pStateCB)(chn, REAL_STREAM_STATE_START, 0, 0);
		}
	}
...
int IPC_Stop(int chn)
{
	...
	*/
	...
	if(g_pStateCB != NULL)
	{
		...
		(g_pStateCB)(chn, REAL_STREAM_STATE_STOP, 0, 0);
	}
...
int IPC_Search(ipc_node** extern_head, unsigned int protocol_type, unsigned char check_conflict)
{
	...
	*extern_head = NULL;
	...
	if(protocol_type & PRO_TYPE_NVR)
	{
		printf("%s PRO_TYPE_NVR\n", __func__);
		
		head_tmp = tail_tmp = NULL;
		rtn = 0;
		
		rtn = NVR_Search(&head_tmp, check_conflict);
...
int IPC_Search(ipc_node** head, unsigned int protocol_type, unsigned char check_conflict)
{
	...
	*head = NULL;
	...
	printf("%s protocol type: 0x%x\n", __func__, protocol_type);

	if(protocol_type & PRO_TYPE_KLW)
	{
		int rtn = KLW_Search(head, &tail, check_conflict);
		if(rtn > 0)
		{
			count += rtn;
		}
	...
	*head = NULL;
	...
	if(protocol_type & PRO_TYPE_NVR)
	{
		...
		NVR_Search(head, check_conflict);
