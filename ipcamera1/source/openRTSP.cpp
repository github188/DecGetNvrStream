...
int ourRTSPClient::Stop(int ExitCode)
{
	if(m_hThread != 0)
	{
		//printf("ourRTSPClient::Stop-1\n");
		
		//LockThread();
		eventLoopWatchVariable = 1;
		//UnLockThread();
		
		//printf("ourRTSPClient::Stop-2\n");
		
		pthread_join(m_hThread, NULL);
		
		//printf("ourRTSPClient::Stop-3\n");
	}
...
void ourRTSPClient::OnThread()
{
	...
	//printf("ourRTSPClient::OnThread-1\n");
	
	envir().taskScheduler().doEventLoop(&eventLoopWatchVariable);
	
	//printf("ourRTSPClient::OnThread-2\n");
	
	if(!m_bExited)
	{
		//printf("ourRTSPClient::OnThread-3\n");
		
		shutdownStream(this, 0);
		
		//printf("ourRTSPClient::OnThread-4\n");
	}
	
	//printf("ourRTSPClient::OnThread-5\n");
	
	m_hThread = 0;
	
	DoCallback(RTSP_FLAG_THREADOVER, channel, 0, 0);
	
	//printf("ourRTSPClient::OnThread-6\n");
}
					...
					struct timeval presentationTime, unsigned /*durationInMicroseconds*/)
{
	...
	{
		printf("rtsp frameSize=%d numTruncatedBytes=%d\n",frameSize,numTruncatedBytes);
	}
	...
	if(p != NULL)
	{
		...
		else if(strcasecmp(fSubsession.mediumName(), "audio") == 0 && strcasecmp(fSubsession.codecName(), "PCMA") == 0)
		{
			/*static int fp = 0;
...
void setupNextSubsession(RTSPClient* rtspClient)
{
	...
	printf("setup next session 2\n");
	
	// We've finished setting up all of the subsessions.  Now, send a RTSP "PLAY" command to start the streaming:
	if(scs.session->absStartTime() != NULL)
	{
		// Special case: The stream is indexed by 'absolute' time, so send an appropriate "PLAY" command:
		printf("sendPlayCommand absStartTime\n");
	...
	else
	{
		...
		scs.duration = 0.0;
		printf("sendPlayCommand duration\n");
...
ourRTSPClient* openURL(UsageEnvironment& env, char const* progName, char const* rtspURL, char *user, char *pwd, unsigned char rtsp_over_tcp, int chn, RealStreamCB pCB, unsigned int dwContext)
{
	...
	printf("ourRTSPClient fuck rtspURL is %s user %s pwd %s\n",rtspURL,user,pwd);
