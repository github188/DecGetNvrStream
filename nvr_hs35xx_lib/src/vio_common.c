...
HI_S32 adjust_vo_ch_attr(VO_DEV VoDev, LIB_VO_MODE_E enMode,int first_Chn,int ChnNum)
{
	...
	if(TL_BOARD_TYPE_NR2116 == TL_HSLIB_TYPE \
		|| TL_BOARD_TYPE_NR3116 == TL_HSLIB_TYPE \
		|| TL_BOARD_TYPE_NR3564 == TL_HSLIB_TYPE)
	{
		u32WndNum = 16;
	}
	...
	printf("%s preview max chn: %d\n", __func__, u32WndNum);
	
	#if 1
	s32Ret = HI_MPI_VO_SetAttrBegin(VoDev);
	if (HI_SUCCESS != s32Ret)
	{
		LIB_PRT("%s HI_MPI_VO_SetAttrBegin failed! ret:0x%x\n", __func__, s32Ret);
		return HI_FAILURE;
	}
	#endif
	...
	if (HI_SUCCESS != s32Ret)
	{
		LIB_PRT("%s HI_MPI_VO_SetAttrEnd failed! ret:0x%x\n", __func__, s32Ret);
		return HI_FAILURE;
	}
	#endif
