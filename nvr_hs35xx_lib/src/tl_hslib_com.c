...
static FNALARMCB g_pAlarmCB = NULL;
...
static HI_S32 SAMPLE_InitMPP(void)
{
	...
	******************************************/
	...
	else if(TL_BOARD_TYPE_NR3564 == TL_HSLIB_TYPE)
	{
		printf("NR3564 config$$$$$$$$$$$$$$$$$$$$$$$$\n");
		//for venc
		stVbConf.u32MaxPoolCnt = 1;
		stVbConf.astCommPool[0].u32BlkSize = 352*288*3/2;
		stVbConf.astCommPool[0].u32BlkCnt = 12;
	}
	else if(TL_BOARD_TYPE_NR3532 == TL_HSLIB_TYPE)
	{
		printf("NR3532 config$$$$$$$$$$$$$$$$$$$$$$$$\n");
		//for venc
		stVbConf.u32MaxPoolCnt = 1;
		stVbConf.astCommPool[0].u32BlkSize = 352*288*3/2;
		stVbConf.astCommPool[0].u32BlkCnt = 12;
	}
	else if(TL_BOARD_TYPE_NR3524 == TL_HSLIB_TYPE)
	{
		printf("NR3524 config$$$$$$$$$$$$$$$$$$$$$$$$\n");
		//for venc
		stVbConf.u32MaxPoolCnt = 1;
		stVbConf.astCommPool[0].u32BlkSize = 352*288*3/2;
		stVbConf.astCommPool[0].u32BlkCnt = 12;
		...
		for (ChnIdx = 0; ChnIdx < 2; ChnIdx++)
		{
			...
			switch (ChnStatus[ChnIdx])
			{
				...
				case StatusChnPreview:
				{
					...
					{
						printf("%s chn%d protocol: %d, ChnStatus: %d\n", __func__, chn, IpcPtl[ChnIdx], ChnStatus[ChnIdx]);
						ChnStatus[ChnIdx] = StatusChnRst;
						continue;
					}
				} break;
...
//外部加锁PREVIEW_LOCK();
//stream_chn 0-127
//return value 0-16 ，无效值0xff
extern int getVdecChn(int stream_chn, pre_mode_e emMode, int nFirstIPC);

int DealStreamState(int chn, real_stream_state_e msg, unsigned int nvrIP, unsigned int alarm_key)
{	
	int cur_vdec_chn = 0xff;//无效值
	...
	int real_chn = chn>=ARG_VI_NUM_MAX ? chn-ARG_VI_NUM_MAX:chn;
	//printf("%s chn%d, real_chn%d, msg: %d\n", __func__, chn, real_chn, msg);

	//NVR 报警
	if (TL_HSLIB_TYPE == TL_BOARD_TYPE_NR3564)
	{				
		if (REAL_STREAM_STATE_ALARM == msg)
		{
			if (ARG_VI_NUM_MAX == 16)//切换型的解码器不响应报警信息
			{
				return 0;
			}
			...
			#endif
				memset(&sAlarmCbData, 0, sizeof(sAlarmCbData));

				if (type == 1)//硬盘丢失错误
				{
					sAlarmCbData.emAlarmEvent = EM_ALARM_EVENT_RemoteNVR_DISK_LOST;
				}
				else if (type == 2)//硬盘读写错误
				{
					sAlarmCbData.emAlarmEvent = EM_ALARM_EVENT_RemoteNVR_DISK_ERR;
				}
				else if (type == 6)//视频丢失报警
				{
					sAlarmCbData.emAlarmEvent = EM_ALARM_EVENT_RemoteNVR_VLOST;
				}
				else
				{
					printf("%s ***Error*** nvr alarm, IP: %s, type: %u invalid\n", 
						__func__, inet_ntoa(in), type);
					return 0;
				}
				
				sAlarmCbData.nChn = id;
				sAlarmCbData.nData = state;
				sAlarmCbData.nTime = time(NULL);
				sAlarmCbData.nvrIP = nvrIP;
				
				g_pAlarmCB(&sAlarmCbData);
	...
	PREVIEW_LOCK();
	//根据预览模式，从流通道找到VDEC通道
	cur_vdec_chn = getVdecChn(chn, lib_gbl_info.last_pre_mode, lib_gbl_info.last_mode_arg);
	if (cur_vdec_chn == 0xff)
	{
		PREVIEW_UNLOCK();
		return -1;
	}

	if (REAL_STREAM_STATE_STOP == msg \
		|| REAL_STREAM_STATE_LOST == msg)
	{
		//VDEC
		ret = HI_MPI_VDEC_StopRecvStream(cur_vdec_chn);
		if(HI_SUCCESS != ret)
		{
			printf("%s ***** Error, HI_MPI_VDEC_StopRecvStream chn%d, real_chn%d ret: 0x%x\n", __func__, chn, real_chn, ret);
			PREVIEW_UNLOCK();
			return -1;
		}
		ret = HI_MPI_VDEC_ResetChn(cur_vdec_chn);
		if(HI_SUCCESS != ret)
		{
			printf("%s ***** Error, HI_MPI_VDEC_ResetChn chn%d, real_chn%d ret: 0x%x\n", __func__, chn, real_chn, ret);
			PREVIEW_UNLOCK();
			return -1;
		}
		//VPSS
		ret = HI_MPI_VPSS_ResetGrp(cur_vdec_chn);
		if(HI_SUCCESS != ret)
		{
			printf("%s ***** Error, HI_MPI_VPSS_ResetGrp chn%d, real_chn%d ret: 0x%x\n", __func__, chn, real_chn, ret);
			PREVIEW_UNLOCK();
			return -1;
		}
		//VO
		ret = HI_MPI_VO_ClearChnBuffer(VO_DEVICE_HD, cur_vdec_chn, HI_TRUE);
		if(HI_SUCCESS != ret)
		{
			printf("%s ***** Error, HI_MPI_VO_ClearChnBuffer chn%d, real_chn%d ret: 0x%x\n", __func__, chn, real_chn, ret);
			PREVIEW_UNLOCK();
			return -1;
		}
		ret = HI_MPI_VO_HideChn(VO_DEVICE_HD, cur_vdec_chn);
		if(HI_SUCCESS != ret)
		{
			printf("%s ***** Error, HI_MPI_VO_HideChn chn%d, real_chn%d ret: 0x%x\n", __func__, chn, real_chn, ret);
			PREVIEW_UNLOCK();
			return -1;
		}

		//VDEC
		ret = HI_MPI_VDEC_StartRecvStream(cur_vdec_chn);
		if(HI_SUCCESS != ret)
		{
			printf("%s ***** Error, HI_MPI_VDEC_StartRecvStream chn%d, real_chn%d ret: 0x%x\n", __func__, chn, real_chn, ret);
			PREVIEW_UNLOCK();
			return -1;
		}

		lib_gbl_info.sVdecSwitchStream[cur_vdec_chn].state = 0;
		lib_gbl_info.sVdecSwitchStream[cur_vdec_chn].PreStreamChn = 0xff;//无效值
	}
	else if (REAL_STREAM_STATE_START == msg)
	{
		#if 0
		ret = HI_MPI_VO_ShowChn(VO_DEVICE_HD, cur_vdec_chn);
		if(HI_SUCCESS != ret)
		{
			printf("%s ***** Error, HI_MPI_VO_ShowChn chn%d, real_chn%d ret: 0x%x\n", __func__, chn, real_chn, ret);
			PREVIEW_UNLOCK();
			return -1;
		}
		#endif
...
void tl_hslib_init(tl_board_type_e type, unsigned char nVidMainNum)
{
	HI_S32 s32Ret;
	//int chn;
	//SIZE_S stSize;
	...
	else if(type == TL_BOARD_TYPE_NR3564)
	{
		b = "NR3564";
	}
	...
	memset(&lib_gbl_info, 0, sizeof(struct lib_global_info));
	...
	lib_gbl_info.pre_preview_mode = lib_gbl_info.last_pre_mode;
	lib_gbl_info.pre_preview_arg =  lib_gbl_info.last_mode_arg;
	...
	#ifdef HI3535
	/*	轮巡型的解码器的使用场景: 通道切换过程中每个通道显示时间不长
			没有必要使用主码流
		切换型的解码器通道添加后一般长时间显示，所以小于等于4画面就
			需要使用主码流
	*/
		#ifdef DDR4G
		//yaogang modify 20140918
	...
	//yaogang modify 20151201 可以4通道主码流
	if(TL_BOARD_TYPE_NR3564 == TL_HSLIB_TYPE)
	{
		if (nVidMainNum == 16)
			lib_gbl_info.preview_chn_threshold = 4;// 4 大于4个窗口则使用子码流预览
		else
	...
	//yaogang modify 20151120
	ARG_PREVIEW_NUM_MAX = ARG_VI_NUM_MAX;
	
	if(TL_BOARD_TYPE_NR3564 == TL_HSLIB_TYPE)
	{
		ARG_PREVIEW_NUM_MAX = 16;
	}
...
//yaogang modify 20150302
	...
	for (chn=ARG_VI_NUM_MAX; chn < ARG_VI_NUM_MAX*2; chn++)
	{
		pthread_mutex_init(&lib_gbl_info.preview_queue_lock[chn], NULL);
		lib_gbl_info.preview_frame_queue[chn].nBufId = 0;
		lib_gbl_info.preview_frame_queue[chn].nLength = 256*1024;//512*1024;
		ret = CreateCircleBuf(&lib_gbl_info.preview_frame_queue[chn]);
		if(0 != ret)
		{
			//fprintf(stderr, "<ERROR-1>hslib_init CreateCircleBuf fail!!!!!!!!!!!!!\n");
...
//yaogang modify 20150302 end
	...
	for(i=0;i<ARG_PREVIEW_NUM_MAX;i++)
	{
		pthread_mutex_init(&plib_gbl_info->preview_chn_lock[i], NULL);
		plib_gbl_info->byDecoderType[i] = 0;
		
		plib_gbl_info->sVdecSwitchStream[i].state = 0;
		plib_gbl_info->sVdecSwitchStream[i].PreStreamChn = 0xff;//无效值
	}
	vio_bind_vi2vo_all(0, ARG_PREVIEW_NUM_MAX, 0, 0, 0);
	nvr_preview_vdec_open(0, ARG_PREVIEW_NUM_MAX);
#endif
...
void tl_set_AlarmCB(void *pAlarmDeal)
{
	g_pAlarmCB = (FNALARMCB)pAlarmDeal;

	return 0;
}

void tl_hslib_init_c(tl_board_type_e type, int client_id, unsigned char nVidMainNum)
{
	g_client_id = client_id;
	printf("%s: type: %d, g_client_id = %d\n", __func__, type, g_client_id);
	
	tl_hslib_init(type, nVidMainNum);
...
{
	pre_mode_e mode = PREVIEW_16CIF;
	int nfirstchn = 0;
	int nPreNum = 16;//当前模式下，预览通道数

	//printf("%s chn%d start\n", __func__, channel);
	if(channel >= ARG_VI_NUM_MAX)
	{
		printf("%s ***Error*** channel(%d) >= ARG_VI_NUM_MAX(%d)\n", \
			__func__, channel, ARG_VI_NUM_MAX);
		return -1;
	}
	
	if(ipcam == NULL)
	{
		printf("%s ***Error*** chn%d ipcam == NULL\n", __func__, channel);
		return -1;
	}

	//yaogang modify 20151201
	PREVIEW_LOCK();
	mode = lib_gbl_info.last_pre_mode;
	nfirstchn = lib_gbl_info.last_mode_arg;
	PREVIEW_UNLOCK();
	...
	switch(mode)
	{
		...
		case PREVIEW_16CIF:
			...
			break;
		default:
			printf("%s chn%d mode: %d, invalid\n", __func__, channel, mode);
	}

	ipcam->bStreamRequest[0] = 0;
	ipcam->bStreamRequest[1] = 1;//默认仅请求子码流
	
	//判断是否需要请求主码流	
	if (ipcam->enable)
	{
		if (nPreNum <= lib_gbl_info.preview_chn_threshold)
		{		
			if (channel >= nfirstchn \
				&& channel < nfirstchn+nPreNum)
			{
				ipcam->bStreamRequest[0] = 1;
			}
		}
	}
