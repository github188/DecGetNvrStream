...
#else
#define ARG_CHN_MAX					64	//support max channel number
#endif
//===========================================================
...
//对VDEC[0-15]通道切换码流通道需要的数据
typedef struct
{
	int state;//状态机
	int PreStreamChn;//上一次写入该VDEC通道的码流通道0xff无效值
	//pthread_mutex_t lock; //使用PREVIEW_LOCK，无须单独锁
}Type_vdec_switch_stream_info;
...
struct lib_global_info
{
	...
	pre_mode_e pre_preview_mode;
	int pre_preview_arg;
	//int last_ipc_chn;
	u8 VdecChnState[ARG_CHN_MAX];
	...
	SCircleBufInfo preview_frame_queue[ARG_CHN_MAX*2];//yaogang
	pthread_mutex_t preview_queue_lock[ARG_CHN_MAX*2];
	SCircleBufInfo PreviewFxnMsgQueue;//yaogang modif 20150306 预览线程的消息队列
	SCircleBufInfo PreviewFxnMsgQueueChn[ARG_CHN_MAX*2];//#ifdef	PreviewFxnPerChn
	...
	Type_vdec_switch_stream_info sVdecSwitchStream[ARG_CHN_MAX];
};
...
#define ARG_PREVIEW_NUM_MAX 		(lib_gbl_info.preview_num_max)
