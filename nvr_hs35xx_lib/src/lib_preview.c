...
void tl_preview(pre_mode_e mode, int chn)
{
	...
	printf("%s - param: mode=%d, chn=%d\n", __func__, mode, chn);
	printf("%s - cur: last_pre_mode=%d, last_mode_arg=%d\n", \
		__func__, lib_gbl_info.last_pre_mode, lib_gbl_info.last_mode_arg);
	...
	{
		//lib_gbl_info.last_ipc_chn = ipc_chn;
		...
		else
		{
			...
			if(!is_nvr_preview_vdec_open())
			{
				...
				vio_set_vo_ch_attr_all(ARG_PREVIEW_NUM_MAX);//ARG_VO_NUM_MAX
				vio_bind_vi2vo_all(0, ARG_PREVIEW_NUM_MAX, 0, 0, 0);//ARG_VO_NUM_MAX
				nvr_preview_vdec_open(0, ARG_PREVIEW_NUM_MAX);//ARG_VO_NUM_MAX
			}
			#else
			if(is_nvr_preview_vdec_open())
			{
	...
	else
	{
		...
		{
			nvr_preview_vdec_close();
			vio_unbind_vi2vo_all(0, ARG_PREVIEW_NUM_MAX);
		}
		#else
		if(is_nvr_preview_vdec_open())
		{
	...
	int nPreNum = 16;//用于把未使用的通道复位，暂时未使用，全部复位
	switch(mode)
	{
		case PREVIEW_1D1:
		{
			...
			adjust_vo_ch_attr(VO_DEVICE_HD, VO_MODE_1MUX, 0, 1);
			nPreNum = 1;
		...
		case PREVIEW_4CIF:
		{
			...
			*/
			adjust_vo_ch_attr(VO_DEVICE_HD, VO_MODE_4MUX, 0, 4);
			nPreNum = 4;
			#else
		...
		case PREVIEW_9CIF:
		{
			...
			*/
			adjust_vo_ch_attr(VO_DEVICE_HD, VO_MODE_9MUX, 0, 9);
			nPreNum = 9;
			#else
			t1 = get_tick();
		...
		case PREVIEW_16CIF:
		{
			...
			*/
			adjust_vo_ch_attr(VO_DEVICE_HD, VO_MODE_16MUX, 0, 16);
			nPreNum = 16;
			#else
			if((chn + 16 <= ARG_VI_NUM_MAX) || (chn == 0))
	...
	//yaogang modify 20151126
	//把未使用的通道复位
	printf("%s rst sVdecSwitchStream nPreNum: %d\n", __func__, nPreNum);
	for (i=0/*nPreNum*/; i<ARG_PREVIEW_NUM_MAX; i++)
	{
		lib_gbl_info.sVdecSwitchStream[i].state = 0;
		lib_gbl_info.sVdecSwitchStream[i].PreStreamChn = 0xff;//无效值
	}
	...
	if (nPreNum <= lib_gbl_info.preview_chn_threshold) //当前预览通道数目
	{
		for (i=0; i<nPreNum; i++)
		{
			mainmask |= 1ULL<<(chn+i);
			printf("%s main stream request chn: %d\n", __func__, chn+i);
		}
	}
	//printf("%s main stream request chnmask: %llu, first chn: %d\n", __func__, mainmask, chn);
	for (i=0; i<max_ch; i++)
	{
		ret = IPC_Get(i, &ipcam);
		if (ret)
		{
			printf("Error %s tl_get_ipcamera %d failed\n", __func__, i);
			return;
		}

		if (ipcam.enable)
		{
			ipcam.bStreamRequest[1] = 1;//默认请求子码流
			
			if (!ipcam.bStreamRequest[0])
			{
				if (mainmask & (1ULL<<i))
				{
					ipcam.bStreamRequest[0] = 1;
					printf("%s main stream IPC_Set chn: %d\n", __func__, i);
					IPC_Set(i, &ipcam); //请求主码流
				}
			}
			...
			if (ipcam.bStreamRequest[0])
			{
				if (!(mainmask & (1ULL<<i)))
				{
					...
					printf("%s main stream IPC_Set close chn: %d\n", __func__, i);
					IPC_Set(i, &ipcam);//关闭主码流
				}
			}
		}
	}
