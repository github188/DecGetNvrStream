...
int nvr_preview_vdec_open(int vdec_first_chn, int vdec_chn_num)
{
	...
	if(vdec_first_chn + vdec_chn_num > ARG_PREVIEW_NUM_MAX)
	{
		vdec_chn_num = ARG_PREVIEW_NUM_MAX - vdec_first_chn;
		if(vdec_chn_num < 0)
		{
			vdec_chn_num = 0;
		}
...
static unsigned int ob_nums[64] = {0};
static unsigned int ob_pts[64] = {0};

static unsigned char g_recreate_flag = 0;

extern int show_vo_chn(u8 VdecChn);
extern int hide_vo_chn(u8 VdecChn);
...
int getVdecChn(int stream_chn, pre_mode_e emMode, int nFirstIPC)
{
	int nChnNum = 16;
	int retChn = 0xff;
	int real_chn = stream_chn<ARG_VI_NUM_MAX ? stream_chn : stream_chn-ARG_VI_NUM_MAX;
	
	if(!is_nvr_preview_vdec_open())
	{
		printf("%s vdec not open\n", __func__);
	
		return 0xff;
	}
	if (emMode == PREVIEW_CLOSE)
	{
		printf("%s emMode == PREVIEW_CLOSE\n", __func__);
		return 0xff;
	}
	...
	switch(emMode)
	{
		...
		case PREVIEW_16CIF:
			...
			break;
		default:
			printf("%s emMode: %d, invalid\n", __func__, emMode);
	}

	if(real_chn >= nFirstIPC && real_chn < nFirstIPC+nChnNum)
	{
		#if 0
		if (real_chn == stream_chn)//主码流
		{
			if (nChnNum == 1)//预览单画面模式
			{
				retChn = real_chn - nFirstIPC;
			}
		}
		else	//子码流
		{
		...
		#else
		if (nChnNum <= lib_gbl_info.preview_chn_threshold)//预览单画面/四画面模式，需要主码流
		{
			if (real_chn == stream_chn)//主码流
			{
				retChn = real_chn - nFirstIPC;
...
int vdec_write(int vdec_chn, vdec_stream_s *pin_stream)
{
	VDEC_STREAM_S stStream;
	HI_S32 s32ret;
	...
	#ifdef HI3535
		s32ret = HI_MPI_VDEC_SendStream(vdec_chn, &stStream, -1);
	#else
		s32ret = HI_MPI_VDEC_SendStream(vdec_chn, &stStream, HI_IO_BLOCK);
	#endif
	
	if(HI_SUCCESS != s32ret)
	{
		printf("%s ***Error*** send to vdecchn%d failed, ret: 0x%x\n", \
			__func__, vdec_chn, s32ret);
		return -1;
	}
...
//码流通道
//本函数关键在与对VDEC通道的处理
static const unsigned char IFrameHead[4] = {0x00,0x00,0x00,0x01};
int nvr_preview_vdec_write(int stream_chn, vdec_stream_s *pin_stream)
{
	int cur_vdec_chn = 0xff;//无效值
	int pre_stream_chn = 0xff;
	u8 wflag = 0;
	
	//pre_mode_e eCurMode = 0;
	//int nCurFirstIPC = 0;
	
	PREVIEW_LOCK();
	//eCurMode = lib_gbl_info.last_pre_mode;
	//nCurFirstIPC = lib_gbl_info.last_mode_arg;

	//根据预览模式，从流通道找到VDEC通道
	cur_vdec_chn = getVdecChn(stream_chn, lib_gbl_info.last_pre_mode, lib_gbl_info.last_mode_arg);
	...
	pre_stream_chn = lib_gbl_info.sVdecSwitchStream[cur_vdec_chn].PreStreamChn;

#if 0
	if (stream_chn == ARG_VI_NUM_MAX)
	{
		printf("%s - state: %d, mode=%d, nCurFirstIPC: %d, CurStreamChn=%d, preStreamChn: %d\n", \
			__func__, lib_gbl_info.sVdecSwitchStream[cur_vdec_chn].state, \
			lib_gbl_info.last_pre_mode, lib_gbl_info.last_mode_arg, stream_chn, pre_stream_chn);
	}
#endif
	switch (lib_gbl_info.sVdecSwitchStream[cur_vdec_chn].state)
	{
		case 0://正常状态
		{
			if (pre_stream_chn == stream_chn)
			{
				wflag = 1;
				break;
			}
			...
			{
				//进入状态1，等待I帧	
				lib_gbl_info.sVdecSwitchStream[cur_vdec_chn].state = 1;
		...
		case 1:
		{
			if(((pin_stream->data[4] & 0x1f) == 0x07) && (memcmp(pin_stream->data, IFrameHead, 4) == 0)) //关键帧
			{
				lib_gbl_info.sVdecSwitchStream[cur_vdec_chn].PreStreamChn = stream_chn;
				//printf("%s show_vo_chn%d\n", __func__, cur_vdec_chn);
				show_vo_chn(cur_vdec_chn);
				wflag = 1;
	...
	if (wflag)
	{
		return vdec_write(cur_vdec_chn, pin_stream);
	}
	return -1;
}
...
//yaogang modify 20151125
int tl_VO_HideChn(int chn)
{
	int ret = HI_SUCCESS;
	
	PREVIEW_LOCK();
	ret = HI_MPI_VO_HideChn(VO_DEVICE_HD, chn);
	PREVIEW_UNLOCK();

	return ret;
}

int tl_VO_HideChns(unsigned int chnBitMask)//通道掩码
{
	int i = 0;
	int s32Ret = HI_SUCCESS;
	int s32Ret2 = HI_SUCCESS;
	
	PREVIEW_LOCK();
	
	s32Ret = HI_MPI_VO_SetAttrBegin(VO_DEVICE_HD);
	if (HI_SUCCESS != s32Ret)
	{
		PREVIEW_UNLOCK();
		printf("%s HI_MPI_VO_SetAttrBegin failed! 0x%x\n", __func__, s32Ret);
		return s32Ret;
	}
	...
	for (i=0; i<ARG_PREVIEW_NUM_MAX; i++)
	{
		if (chnBitMask & (1<<i))
		{
			s32Ret2 = HI_MPI_VO_HideChn(VO_DEVICE_HD, i);
	...
	s32Ret = HI_MPI_VO_SetAttrEnd(VO_DEVICE_HD);
	if (HI_SUCCESS != s32Ret)
	{
		PREVIEW_UNLOCK();
		printf("%s HI_MPI_VO_SetAttrEnd failed! 0x%x\n", __func__, s32Ret);
		return s32Ret;
	}
	...
	if (HI_SUCCESS != s32Ret2)
	{
		printf("%s HI_MPI_VO_HideChn: %d failed, ret: 0x%x\n", \
			__func__, i, s32Ret);
	}
...
int tl_VO_ShowChn(int chn)
{
	int ret = HI_SUCCESS;
	
	PREVIEW_LOCK();
	ret = HI_MPI_VO_ShowChn(VO_DEVICE_HD, chn);
	PREVIEW_UNLOCK();

	return ret;
}

int tl_VO_ShowChns(unsigned int chnBitMask)//通道掩码
{
	int i = 0;
	int s32Ret = HI_SUCCESS;
	int s32Ret2 = HI_SUCCESS;
	...
	for (i=0; i<ARG_PREVIEW_NUM_MAX; i++)
	{
		if (chnBitMask & (1<<i))
		{
			s32Ret2 = HI_MPI_VO_ShowChn(VO_DEVICE_HD, i);
			if (HI_SUCCESS != s32Ret2)
			{
				break;				
			}
	...
	if (HI_SUCCESS != s32Ret2)
	{
		printf("%s HI_MPI_VO_ShowChn: %d failed, ret: 0x%x\n", \
			__func__, i, s32Ret);
	}
